"""Resumable qrels-blind N12 SwiGLU stage bundles for Q2/Q3."""
...
N12_MANIFEST_PATH = Path(
    "experiments/motivation/transformer_n12_mlp_stage_manifest_v1.yaml"
)
N12_MANIFEST_SHA256 = "e700cd2413b0176f1a0c1b16ca0309574741e2c0d4841eb31d298e9535f185ac"


def write_mlp_stage_bundle(
    ...
    *,
    ...
    manifest_path: str | Path = N12_MANIFEST_PATH,
    resume: bool = False,
    ...
    if block not in FIXED_BLOCKS:
        raise ValueError("N12 MLP stage block must be one of 13,20,27")
    if max_requests is not None and int(max_requests) <= 0:
        raise ValueError("N12 max_requests must be positive")
    if not str(device).strip():
        raise ValueError("N12 requires an explicit device")
    max_wall_seconds = float(max_wall_seconds)
    if not math.isfinite(max_wall_seconds) or not 0 < max_wall_seconds <= MAX_WALL_SECONDS:
        raise ValueError("N12 max_wall_seconds must be in (0,13500]")

    standardized_dir = Path(standardized_dir)
    config_path = Path(config_path)
    checkpoint_root = Path(checkpoint_root)
    run_dir = Path(runs_dir) / run_id
    manifest = _load_n12_manifest(manifest_path)
    ...
    for path, expected in (
        ...
        if not path.is_file() or sha256_file(path) != str(expected):
            raise ValueError(f"N12 frozen input hash mismatch: {path}")
    records = [sanitize_record_for_model(row) for row in iter_jsonl(records_path)]
    if len(records) != 8000:
        raise ValueError("N12 requires all 8000 internal-dev requests")
    config = load_v12_ranker_config(config_path)
    method_id = str(config["method_id"])
    if method_id not in SUPPORTED_METHODS:
        raise ValueError("N12 supports only Q2/Q3")
    binding = manifest["model_bindings"][method_id]
    if config["_config_sha256"] != binding["config_sha256"]:
        raise ValueError("N12 config differs from frozen binding")
    ...
    if checkpoint_id != binding["checkpoint_id"] or training_metadata.get("checkpoint_id") != checkpoint_id:
        raise ValueError("N12 checkpoint differs from frozen binding")
    ...
    metadata = {
        "schema_version": 1,
        "analysis_stage": "transformer_deep_dive_n12_mlp_stage_operator",
        "run_id": run_id,
        "method_id": method_id,
        "block_zero_based": block,
        "score_conditions": list(MLP_STAGE_CONDITIONS),
        "active_conditions": list(MLP_STAGE_CONDITIONS[4:]),
        "identity_conditions": ["full_gate_identity", "null_gate_identity"],
        "checkpoint_id": checkpoint_id,
        ...
        "request_manifest_sha256": sha256_file(standardized_dir / "request_manifest.json"),
        "dataset_manifest_sha256": sha256_file(standardized_dir / "manifest.json"),
        "n12_manifest_path": str(manifest_path),
        "n12_manifest_sha256": manifest["_sha256"],
    ...
    prepared = prepare_scalar_bundle(
        ...
        conditions=MLP_STAGE_CONDITIONS,
        resume=resume,
    )
    ...
    if completed >= len(target_records):
        return finalize_scalar_bundle(
            run_dir, prepared, target_records, MLP_STAGE_CONDITIONS,
            ...
            for ordinal in range(completed, len(target_records)):
                ...
                for start in range(0, len(record.candidates), batch_size):
                    ...
                    for local, candidate in enumerate(candidates):
                        values = {name: float(result["conditions"][name][local]) for name in MLP_STAGE_CONDITIONS}
                        if not all(math.isfinite(value) for value in values.values()):
                            raise FloatingPointError("N12 produced a non-finite score")
...
def mlp_stage_runtime_implementation_identity() -> dict[str, Any]:
    root = Path(__file__).resolve().parents[3]
    relative_paths = (
        "src/myrec/mechanism/mlp_stage_interventions.py",
        "src/myrec/mechanism/mlp_stage_scoring.py",
        "src/myrec/mechanism/mlp_stage_runtime.py",
        ...
        "scripts/score_deep_dive_mlp_stage.py",
    )
    files = []
    for relative in relative_paths:
        path = root / relative
        if not path.is_file():
            raise FileNotFoundError(f"N12 implementation file is missing: {path}")
...
def _load_n12_manifest(path: str | Path) -> dict[str, Any]:
    path = Path(path)
    observed = sha256_file(path)
    if observed != N12_MANIFEST_SHA256:
        raise ValueError("N12 manifest differs from immutable digest")
    value = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("N12 manifest is not a mapping")
