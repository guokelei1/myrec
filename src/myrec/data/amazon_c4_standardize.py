...
def build_standardized_amazon_c4(
    *,
    ...
    output_dir: str | Path,
    report_path: str | Path | None = "reports/history_response_gap_amazon_c4_data_audit.json",
    dataset_version: str = "history_bm25_100_v1",
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"Amazon-C4 output directory is not empty: {output_dir}")
    ...
    materialize_request_manifest(
        [
            (split, output_dir / f"records_{split}.jsonl")
            for split in ("train", "dev", "test")
        ],
        request_manifest_path,
        dataset_version=dataset_version,
    )
    output_stats["files"]["request_manifest"] = _file_info(request_manifest_path)
    ...
    manifest = {
        ...
        "dataset_version": dataset_version,
        "evidence_mode": "exploratory_independent_dataset_stress_test",
...
def _write_standardized_files(
    ...
    *,
    ...
    paths = {
        ...
        "qrels_train": output_dir / "qrels_train.jsonl",
