#!/usr/bin/env python3
"""Build the qrels-blind fixed-grid D3 Q/K stage-geometry synthesis."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="runs/20260719_kuaisearch_mech_d3_qk_geometry_v3",
    )
    args = parser.parse_args()
    input_path = Path(args.input)
    output_dir = Path(args.output_dir)
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"Q/K geometry output is not empty: {output_dir}")
    output_dir.mkdir(parents=True, exist_ok=True)
    source = json.loads(input_path.read_text(encoding="utf-8"))
    result = summarize_attention_qk_geometry(source)
    result.update(
        {
