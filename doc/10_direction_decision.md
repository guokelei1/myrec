...
状态：方向冻结；motivation complete；design formulation active；正式实现/训练
由新的 design-specific pre-outcome gate 把关。

2026-07-10 审计注：PPS 场景与数据轨道决策不变，但 §2.2 的 same-label
per-request oracle falsification 不具构念有效性；Random 通道可产生更高
headroom。系统方向不再由该 oracle 自动解锁，见
`reports/pps_m3_m4_random_canary_audit.json`。
后续 `doc/17` 在看结果前锁定了替代构念；matched wrong-user history 控制通过。
当前 insight 与设计授权始终以最新的 `reports/pps_c5_insight_audit.json` 为准。

2026-07-10 C5-R2 supersession：原控制把 rolling true history 与 train-frozen
wrong history 比较，未隔离 freshness/catalog period。`doc/22` 的时间匹配修复
保留了广义 correct-history 正增益，但 same-query + freshness-balanced gate
只有 1/3 seed 显著，冻结规则未通过。PPS 场景与三条数据轨道决策不变；
在 C5-R2 时点，query-anchored personalized residual 变为**待验证候选方向**，
不再是已授权的下一阶段。权威结果见
`reports/pps_c5r2_temporal_symmetric_identity.json`。

2026-07-10 C5-R3 interpretation amendment（当前有效）：`doc/23` 在任何组件结果
产生前冻结了 item/category 精确分解、multi-granular primary、唯一 coarse
semantic fallback 与失败终局。item-only 稳定超过 D2p，三 seed history-present
差值为 +0.03204/+0.03214/+0.03263；category-only 0/3 显著；full D2s 在三
seed 均显著弱于 item-only。primary 与 fallback 均失败，故这两条预注册机制
不能作为已验证 primitive；但该负结果不能外推成禁止所有设计。motivation 已
完整收口，并建立了新的设计问题：历史证据的经验可信度不均，现有方法没有
可靠地区分 exact recurrence 与有害的跨商品迁移。当前允许进入
architecture/protocol formulation；正式实现与训练须先通过 design-specific
pre-outcome falsifier。item-only 三 seed mean 0.3453755 成为静态水线。权威
阶段裁决见 `reports/pps_c5_insight_audit.json`，权威数值见
...
Architecture consequence: 设计一个 candidate-conditioned evidence-fidelity
             calibration primitive：保存可靠的 exact recurrence，只在
...
4. **原 recovery ladder 的有限廉价证伪已完整结束**（2.2 节）。它否定了
   multi-granular/coarse-category 候选，却留下可承重的 evidence-fidelity
   failure insight。下一步是把该 insight 写成单一 primitive 和独立 gate，
   而不是继续为旧 claim 追加事后修复。
...
| 33.6% 请求 history 为空，非空 history 也可能偏离当前候选域 | residual 必须有显式 missing-evidence mask；no-history 精确退化为 query base，并用 event permutation / wrong-history 控制归因 |
...
1. C0-C5-R3 motivation 与 representative baselines 已完成，冻结结果见
   `experiments/pps_results.md`。
2. 立即进入 proposed-system **design formulation**：定义单一
   evidence-fidelity primitive、信息流、最近邻差异、matched-capacity controls
   与效率预算；具体约束见 `doc/15_proposed_system_design_principles.md`。
3. 在任何正式实现/训练前，冻结并执行 design-specific pre-outcome falsifier；
   只有通过 repeat/non-repeat、wrong/shuffled/query-mask 与 no-history 条件，
   才授权完整模型训练和 dev 调参。
4. 论文叙事同时保留 benchmark 发现与设计过渡：exact-repeat shortcut、category
   negative result、no-history fallback 共同导出 evidence-fidelity 问题，而不
   宣称新 primitive 已验证。
5. test 继续保持封存；不能用于选择、确认或挽救 motivation 或设计 premise。
