...
状态：执行计划。前置：`10_direction_decision.md`（方向决策）、`07_paper_design_constraints.md`（约束，本文严格遵守其 Tier 1）。
...
4. 两个候选 insight（供后续验证，不在动机阶段实现）。

全流程分 6 个 Phase，每个 Phase 结束有一个 checkpoint（C0–C5）。**checkpoint 不通过不得进入下一 Phase**，检查结果一律落盘为 `reports/pps_<phase>_audit.json`（沿用本仓库 VERA 阶段的 gate 报告习惯）。
...
## Phase 5 — 建模与 Tier-2 收尾（简述，通过 C5 后另立设计文档）

1. 按存活的 insight 设计轻量模型（gate / facet 路由 + 轻量 ranker），**参数量对齐控制组**必须同步建（同参数量的无 gate 版本，07 §6.1）;
...
Phase 5  建模+Tier-2+finals

回退链：C0 fail → JDsearch 主轨；C3 fail → Amazon-C4 重跑动机；
        C4 后 insight 全灭 → benchmark+分析论文。
```
