...
状态：motivation phases complete；Phase 5A design formulation active；Phase 5B
implementation/training gated。前置：`10_direction_decision.md`（方向决策）、
`07_paper_design_constraints.md`（约束，本文严格遵守其 Tier 1）。

2026-07-10 构念审计注：本文原始 C3/M3/M4 判据冻结保留，但 Random 通道可
同时超过 M3 oracle headroom 与 M4 AUC，说明原判据未控制 same-label argmax
选择噪声。原 C3 的正向 claim 当时暂停并等待显式协议修订；证据见
`reports/pps_m3_m4_random_canary_audit.json`。当时 C5 仍是建模前置；紧随其后的
C3-R/C5-R amendment 已取代这一临时暂停状态。

2026-07-10 C5-R2 amendment（历史，已由 C5-R3 收口）：外部构念审计发现原 C5-R 的 true
history 为 rolling dev-as-of，而 wrong history 只来自更早 train，身份与
freshness/catalog period 同时变化。`doc/22` 在读取修复结果前冻结了
freshness-matched prequential 控制。实现与完整结果见
`reports/pps_c5r2_temporal_symmetric_identity.{json,md}`。时间不对称已被显式
约束，D2s 数值水线仍有效；但 same-query + freshness-balanced 子集只有 1/3
seed 显著，未达到冻结的 2/3 规则。因此 **C5-R2 failed，当前不得进入 Phase
5 正式建模**。下述原 C5-R pass 作为历史路径保留，不再提供设计授权。

2026-07-10 C5-R3 scope amendment（当前有效）：`doc/23` 在组件结果产生前
冻结 B0b item/category 分解、primary、唯一 fallback 与 benchmark/analysis-only
失败终局。575,609 个候选行分解审计通过。item-only vs D2p 在 3/3 seed 显著，
category-only 在 0/3 seed 显著，full D2s 在 3/3 seed 显著弱于 item-only。
primary 与 fallback 均失败，故 **C5-R3 recovery ladder 按冻结规则终止**，
不得把这两条候选机制事后改写为通过。该终局不等于全局禁止设计：motivation
已经完成，并建立“可靠 exact recurrence 与不可靠 transfer 必须被区分”的
evidence-fidelity failure insight。Phase 5A architecture/protocol formulation
现在可启动；Phase 5B 正式实现/训练仍须先通过新冻结的 design-specific
pre-outcome gate。item-only mean NDCG@10 0.3453755 成为当前静态水线。权威
阶段解释见 `reports/pps_c5_insight_audit.json`，权威结果见
`reports/pps_c5r3_candidate_history_alignment.{json,md}`。

2026-07-10 C3-R/C5-R amendment（历史）：替代协议在查看新控制结果前锁定于 `doc/17`，
随后 matched wrong-user history gate 全部通过（`reports/pps_c3r_history_identity_control.json`）。
当时的正向 insight 改为 **query-anchored personalized residual**：query-candidate
排序是底座，target-user history 只产生 masked、candidate-specific residual；
history 缺失时严格退化为 query-only。原 C3/M3/M4、Insight-1/Insight-2 记录
永久保留，但不再是当前建模前置或设计依据。此 amendment 只授权 residual
方向，不授权 channel oracle/router、槽位互补或 Consensus Law。
...
任何候选 primitive，但它建立了可进入设计推导的 failure mode：历史证据的
经验可信度不均，未经校准的 transfer 会污染可靠 recurrence。这个 observation
授权 Phase 5A formulation；候选 architecture consequence 仍是待证假设，不能
在新的 design-specific gate 前进入正式训练。

---

## Phase 5 — Proposed-system design、建模与 Tier-2 收尾

当前状态：**Phase 5A design formulation 可立即启动；Phase 5B
implementation/training 尚未授权**。C5-R3 的两条候选路径保持失败；新设计必须
从该负结果暴露的 evidence-fidelity 问题出发，不能给旧 claim 换名。

### 5A. Architecture / protocol formulation（当前已授权）

1. 将 candidate-conditioned evidence-fidelity calibration 写成一个数学明确的
   单一 primitive，并标注它是待证假设；
2. 给出 query/history/candidate 信息流、与 DIN/ZAM/TEM/sequence retrieval/
   fixed mixture 的最近邻差异，以及 matched-capacity control；
3. 冻结一个 **pre-implementation design gate**，至少覆盖：
   - repeat-present 作用面不得弱于 item-only；
   - 4,677 个 non-repeat history-present 请求上必须稳定优于 D2p；
   - coarse-only、wrong-user、shuffled-event、query-masked evidence 不得复现；
   - 4,110 个 no-history 请求须逐请求 rank-equivalent 于 D2p；
4. 在协议中冻结阈值、seed、预算、stop-loss、候选哈希、日志条数和 test 禁令；
5. 只允许为该 gate 编写最小 probe/对照，不得借此开展完整系统调参。

### 5B. Full implementation / training（通过新 gate 后才授权）

1. 模型必须显著超过 item-only 0.3453755，而非只超过 full D2s/D2p，并以
   **参数量对齐控制组**同步验证（07 §6.1）；
2. 每个模块预登记 ablation（付租金原则，07 §14）；
3. C4 副轨 + JDsearch 锚点复测（unified 主张的证据）；
4. Tier-2 一次性补齐：3 seeds、baseline 等预算调参日志、latency（含在线特征构造）、wrong-query/no-query 因果控制（07 §6.2）、behavior-preservation 不适用本场景可免；
5. test 集只跑一次，跑之前冻结全部 config。
...
Phase 5A  design/protocol formulation ──新 design gate──►
Phase 5B  implementation+training+Tier-2+finals

回退链：C0 fail → JDsearch 主轨；C3 fail → Amazon-C4 重跑动机；
        C5-R3 primary/fallback fail → evidence-fidelity design formulation；
        新 design gate fail → benchmark+分析论文，不启动完整 system。
```
