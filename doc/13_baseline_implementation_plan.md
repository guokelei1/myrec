...
当前修订：C01--C80 已终止，`doc/24` 不再提供 execution authorization。baseline
身份、历史数值和公平性矩阵保留；proposed-system 研发改走 `doc/31`。普通
full-token joint Transformer 现在是必须正常调优的 strong baseline，而不是只运行
一次的 diagnostic 或默认配置 control。其 dev feedback budget 与其他 trainable
baseline 对称。R0 Round 1 修订后，“正常调优”只说明过程公平；只有同时通过
`doc/31 R0-C0` 的 model-family、base-preservation 和 ranking-competitiveness gate，
才能使用 strong baseline 身份。

历史构念边界仍有效：same-label per-request oracle 必须有 Random null 或独立
selection/evaluation；request-time history 必须 strictly prior；true/wrong history
采用对称可用信息策略；item-only 0.3453755 是 KuaiSearch 静态历史水线但不是
architecture primitive。历史细节见 `doc/17`--`doc/23` 和对应 reports。
...
| trainable（B3-ft/B4/B5/B6/B6+/ordinary full-token/Hxx） | outcome 前按搜索维度与成本声明，**默认 ceiling 16 configuration trials**；controller 对单 baseline family 默认 12 | 小型固定 recipe 通常 4--8 configs 且不要求用完；Hxx 还须先通过 doc/31 Failure Card；score-affecting config 全计数 |
| evaluator invocations | 与 configuration trial 分账并全部写 log | 调参期只打开当前决策必需的 surface；四 scenario 不能被误算成四个 configuration trials |
| 多 seed（Tier-2） | 最优 config × 3 seeds | 只在 model family、Motivation Brief 与 config 均存活并冻结后做；seed 不算调参次数 |

预算不对称（例如官方代码只能跑默认配置）必须写进 baseline card 和论文，
不许事后声称"都调过了"。

若配置差异小于预先声明的 MDE 或 paired CI 跨零，selection rule 优先更简单、成本更低
且 no-history/base preservation 更好的配置，不允许仅按最高 dev 点估计选 winner。
