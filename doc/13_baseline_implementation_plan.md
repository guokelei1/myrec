...
| trainable（B3-ft/B4/B5/B6/B6+/ordinary full-token/Hxx） | 每方法 **16 次 dev 评测**，搜索空间先写进 config 再开跑 | Hxx 还须先通过 doc/31 Failure Card；mechanical retry 不调用 evaluator且不改变 score 语义时不计数，其余 trial 全计数 |
