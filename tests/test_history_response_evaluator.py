...
class HistoryResponseEvaluatorTest(unittest.TestCase):
    def test_bundle_identity_and_dev_log(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                (runs / "analysis" / "target_aware_surfaces" / "manifest.json").exists()
            )
            self.assertEqual(result["target_aware_surface_counts"]["target_repeat"], 1)
            self.assertEqual(len(log.read_text(encoding="utf-8").splitlines()), 1)

    def test_confirmation_bundle_appends_to_supplied_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            runs = root / "runs"
            standardized = root / "standardized"
            ...
            _write_jsonl(
                standardized / "records_confirmation.jsonl",
                [
                    {
                        "request_id": "r",
                        "history": [{"item_id": "a"}],
