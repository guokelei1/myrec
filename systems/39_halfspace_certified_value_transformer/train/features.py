"""Label-free C39 structure collection and frozen BGE state encoding."""
...
def collect_label_free_features(
    *,
    ...
    if sha256_file(records_path) != selection["records_sha256"]:
        raise RuntimeError("C39 blind records changed after selection")
    ...
    for role in role_names:
        ...
        if candidate_key_sha256(records, indices) != row["candidate_key_sha256"]:
            raise RuntimeError(f"C39 candidate hash changed for {role}")
        selected_indices.extend(indices)
    if len(selected_indices) != len(set(selected_indices)):
        raise ValueError("C39 feature roles overlap")
...
def encode_feature_shard(
    *,
    ...
    if not (0 <= shard_id < num_shards):
        raise ValueError("invalid C39 embedding shard")
...
def finalize_embeddings(
    *,
    ...
    for raw_path in shard_paths:
        ...
        with np.load(path, allow_pickle=False) as shard:
            ...
            if item_seen[item_positions].any() or query_seen[query_positions].any():
                raise ValueError("C39 embedding shards overlap")
    ...
    if not item_seen.all() or not query_seen.all():
        raise ValueError("C39 embedding shards do not cover every position")
