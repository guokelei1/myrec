"""Create immutable C39 proposal and execution locks."""
...
def relative_repo(path: str | Path) -> str:
    ...
    if REPO_ROOT.resolve() not in resolved.parents:
        raise ValueError(f"C39 lock input is outside the repository: {path}")
...
def freeze_proposal(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if design_gate.get("status") != "passed":
        raise PermissionError("C39 design gate has not passed")
    if (
        sha256_file(config["paths"]["design_gate_report"])
        != config["paths"]["design_gate_report_sha256"]
    ):
        raise RuntimeError("C39 design-gate report changed")
    selection = read_json(config["paths"]["selection"])
    if selection["label_access"]["records_train_labels_opened"] is not False:
        raise PermissionError("C39 train labels opened before proposal lock")
    isolation = selection["outcome_isolation"]
    if isolation["overlap_with_c38_internal_A_delayed_B_escrow"] != 0:
        raise PermissionError("C39 A overlaps a C38 target role")
    if isolation["c39_internal_A_previously_featured_scored_labeled"] is not False:
        raise PermissionError("C39 A is not outcome-untouched")
    ...
    value = {
        ...
        "declarations": {
            ...
            "novelty_not_established": True,
            "kuaisearch_confirmation_required_after_any_A1_pass": True,
        },
        ...
        "lock_id": "c39_halfspace_certified_value_transformer_v1",
...
def freeze_execution(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if g0.get("status") != "passed":
        raise PermissionError("C39 G0 has not passed")
    if g0.get("internal_A_labels_scores_opened") is not False:
        raise PermissionError("C39 A opened before execution lock")
    value = {
        "candidate_id": "c39",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "declarations": {
        ...
        "lock_id": "c39_halfspace_certified_value_execution_v1",
