...
def test_config_literals_and_access_boundary() -> None:
    ...
    assert config["seeds"] == [20260724, 20260725, 20260726]
    assert config["training"]["steps"] == 600
    assert config["model"]["solver_steps"] == 16
...
def test_two_step_cpu_smoke_is_finite() -> None:
    ...
    for mode in ("cone", "span", "relu1", "simplex", "pooled_mlp"):
