"""C47 selection v3 with physical JSONL lines and positive-polarity checks."""
...
def build(config_path: str, *, require_v3_lock: bool) -> tuple[dict, Path]:
    ...
    records = load_physical_jsonl(REPO_ROOT / paths["amazon_records_train_blind"])
    ...
    return result, REPO_ROOT / paths["selection"]


def main(config_path: str) -> dict:
    result, output = build(config_path, require_v3_lock=True)
...
if __name__ == "__main__":
    ...
    print(json.dumps({"candidate_id": "c47", "status": "selection_materialized_v3", "checks": value["checks"]}, sort_keys=True))
