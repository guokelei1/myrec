"""Materialize label-free C31-A features and compact fit labels after proposal lock."""
...
def assert_cuda(config: Mapping[str, Any], device: str) -> None:
    ...
    if device != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C31 G0 GPU registration mismatch")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C31 G0 requires exactly one visible CUDA GPU")
...
def validate_registered_sources(config: Mapping[str, Any]) -> None:
    ...
    for name, expected_name in (
        ...
        ("c29_selection", "c29_selection_sha256"),
        ("c29_g0_report", "c29_g0_report_sha256"),
        ("c29_train_report", "c29_train_report_sha256"),
        ("c30_report", "c30_report_sha256"),
    ):
        if sha256_file(paths[name]) != paths[expected_name]:
            raise ValueError(f"C31 registered source changed: {name}")
    c29_g0 = read_json(paths["c29_g0_report"])
    c29_outcome = read_json(paths["c29_train_report"])
    c30_outcome = read_json(paths["c30_report"])
    if c29_g0.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C31 inherited A was previously feature-materialized")
    if c29_outcome.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C31 inherited A boundary differs")
    if c30_outcome.get("delayed_B_escrow_dev_test_opened") is not False:
        raise PermissionError("C31 inherited delayed roles differ")
...
def materialize(config_path: str | Path, device: str) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C31 G0 exists")
    ...
    if len(feature_indices) != len(set(int(value) for value in feature_indices)):
        raise AssertionError("C31 feature roles overlap")
    ...
    if checkpoint.get("analysis_id") != "finetuned_nonpersonalized_control_v1":
        raise ValueError("C31 D2 checkpoint identity differs")
    if int(checkpoint.get("seed", -1)) != 20260708:
        raise ValueError("C31 D2 checkpoint seed differs")
    model = build_model(d2, device)
    model.load_state_dict(checkpoint["model_state"], strict=True)
    model.eval()
    # D2p keeps its registered fine-tuned query tower.  C31 transport must use
    ...
    if sha256_file(paths["internal_train_popularity"]) != config["integrity"][
        ...
        raise ValueError("C31 popularity differs")
        ...
        for start in range(0, len(feature_indices), batch_size):
            ...
            for row, raw_index in enumerate(requests):
                ...
                if not np.array_equal(item_indices[positions], candidates):
                    raise RuntimeError("C31 selected item lookup differs")
                ...
                if not np.isfinite(mixed).all():
                    raise ValueError("nonfinite C31 D2p")
