"""Create immutable C31 proposal and post-G0 execution locks."""
...
def external_paths(config: dict[str, Any]) -> list[Path]:
    ...
    if outside:
        raise ValueError(f"C31 external input outside repository: {outside}")
    return result
...
def freeze_proposal(config: dict[str, Any]) -> dict[str, Any]:
    ...
    value = {
        ...
        "declarations": {
            "c29_fit_labels_previously_opened": True,
            "c30_A_labels_previously_opened_for_diagnostic": True,
            "c30_A_reused_for_c31_fit_or_gate": False,
            "c31_fit_new_labels_opened": False,
            "c31_internal_A_features_labels_scores_opened": False,
            "c31_delayed_B_features_labels_scores_opened": False,
            "c31_ranking_outcome_observed": False,
        ...
        "lock_id": "c31_authenticated_collaborative_query_transport_v1",
        "selection_path": relative_repo(config["paths"]["selection"]),
        "selection_sha256": config["paths"]["selection_sha256"],
        "status": "locked_before_c31_A_feature_label_score_or_outcome",
    }
...
def freeze_execution(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if report.get("status") != "passed":
        raise PermissionError("C31 G0 is not a pass")
    if report.get("internal_A_labels_opened") is not False:
        raise PermissionError("C31 A labels opened before execution lock")
    if report.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C31 delayed-B boundary differs")
    value = {
        "candidate_id": "c31",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "declarations": {
        ...
        "lock_id": "c31_authenticated_collaborative_query_transport_execution_v1",
