"""Frozen C32 embeddings, causal authentication, and staged label access."""
...
class CompactLabels:
    ...
    def __post_init__(self):
        if len(self.offsets)!=len(self.request_indices)+1 or int(self.offsets[-1])!=len(self.values): raise ValueError('C32 compact labels differ')
    @property
    ...
    def rows(self,indices:Sequence[int],counts:Sequence[int])->list[np.ndarray]:
        ...
        for raw,count in zip(indices,counts):
            index=int(raw)
            if index not in pos: raise PermissionError(f'C32 label unavailable: {index}')
            row=pos[index]; start,stop=int(self.offsets[row]),int(self.offsets[row+1])
            if stop-start!=int(count): raise ValueError('C32 label count differs')
...
def open_original_labels(*,data:PackedStructure,indices:Sequence[int],path:str|Path,selection_path:str|Path,selection_sha256:str)->CompactLabels:
    if sha256_file(selection_path)!=selection_sha256: raise RuntimeError('C32 selection changed before labels')
...
class FrozenTransportStore:
    def __init__(self,config:Mapping[str,Any]):
        ...
        if len(self.feature_position)!=len(self.feature_indices): raise ValueError('C32 feature requests overlap')
        if not np.array_equal(self.feature_indices,self.auth_request_indices): raise ValueError('C32 feature/auth requests differ')
        if len(self.score_offsets)!=len(self.feature_indices)+1 or int(self.score_offsets[-1])!=len(self.base_scores): raise ValueError('C32 score offsets differ')
        expected_dim=int(config['model']['embedding_dim'])
        if self.query_embeddings.shape!=(len(self.feature_indices),expected_dim): raise ValueError('C32 query embedding shape differs')
        if self.raw_items.ndim!=2 or self.raw_items.shape[1]!=expected_dim: raise ValueError('C32 item embedding shape differs')
    ...
    def base_row(self,index):
        ...
        if stop-start!=self.candidate_count(int(index)): raise ValueError('C32 base-score candidate count differs')
    ...
    def authenticated_history(self,index,source):
        ...
        raise ValueError('C32 history source differs')
