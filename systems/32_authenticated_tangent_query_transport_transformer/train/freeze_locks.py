"""Create immutable C32 proposal and post-G0 execution locks."""
...
def external_paths(config: dict[str, Any]) -> list[Path]:
    ...
    if outside:
        raise ValueError(f"C32 external input outside repository: {outside}")
    return result
...
def freeze_proposal(config: dict[str, Any]) -> dict[str, Any]:
    ...
    value = {
        ...
        "declarations": {
            "c31_fit_labels_previously_opened": True,
            "c31_A_labels_previously_opened_for_diagnostic": True,
            "c31_A_reused_for_c32_fit_or_gate": False,
            "c31_delayed_B_features_labels_scores_opened": False,
            "c32_fit_new_labels_opened": False,
            "c32_internal_A_features_labels_scores_opened": False,
            "c32_delayed_B_features_labels_scores_opened": False,
            "c32_ranking_outcome_observed": False,
        ...
        "lock_id": "c32_authenticated_tangent_query_transport_v1",
        "selection_path": relative_repo(config["paths"]["selection"]),
        "selection_sha256": config["paths"]["selection_sha256"],
        "status": "locked_before_c32_A_feature_label_score_or_outcome",
    }
...
def freeze_execution(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if report.get("status") != "passed":
        raise PermissionError("C32 G0 is not a pass")
    if report.get("internal_A_labels_opened") is not False:
        raise PermissionError("C32 A labels opened before execution lock")
    if report.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C32 delayed-B boundary differs")
    value = {
        "candidate_id": "c32",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "declarations": {
        ...
        "lock_id": "c32_authenticated_tangent_query_transport_execution_v1",
