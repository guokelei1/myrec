...
def bootstrap(values: np.ndarray, *, samples: int, seed: int) -> dict[str, Any]:
    ...
    if values.ndim != 1 or not len(values) or not np.isfinite(values).all():
        raise ValueError("C32 bootstrap values invalid")
    ...
    for offset, (name, reference) in enumerate(references.items()):
        ...
        for fold in range(folds):
            selected = np.asarray(
                [
                    int.from_bytes(
                        hashlib.sha256(f"c32-fold:{seed}:{request_id}".encode()).digest()[:8],
                        "big",
                    )
                    % folds
                    == fold
...
def clicked_direction(corrections: Sequence[np.ndarray], labels: Sequence[np.ndarray]) -> np.ndarray:
    ...
    if not output:
        raise ValueError("C32 clicked-direction surface empty")
