"""Parallel-seed, staged C32 train-only architecture gate."""
...
def assert_cuda(config: Mapping[str, Any], seed: int, device: torch.device) -> int:
    ...
    if seed not in seeds:
        raise ValueError(f"unregistered C32 seed: {seed}")
    physical = physicals[seeds.index(seed)]
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C32 seed/GPU registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C32 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C32 seed process requires exactly one visible GPU")
    return physical
...
def model_identity(model: LowRankQueryTransport) -> dict[str, Any]:
    return {
        "architecture": "shared_low_rank_spherical_tangent_query_transport",
...
def training_rows(
    ...
    for index in store.role_indices("fit"):
        ...
        if stop - start != count:
            raise ValueError("C32 fit label count differs")
...
def train_primary(
    ...
    if listwise_weight <= 0 or direction_weight <= 0:
        raise ValueError("C32 fixed loss weights must be positive")
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("nonfinite C32 training loss")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C32 gradient: {name}")
...
def write_npz_once(path: Path, **arrays: np.ndarray) -> None:
    if path.exists():
        raise FileExistsError(f"immutable C32 score artifact exists: {path}")
...
def candidate_hashes(store: FrozenTransportStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C32 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C32 checkpoint exists: {path}")
    temporary = path.with_suffix(path.suffix + ".tmp")
    torch.save(
        {
            "candidate_id": "c32",
...
def prepare(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if attempt_path.exists() or report_path.exists():
        raise FileExistsError("C32 formal attempt already exists")
...
def run_seed(config: Mapping[str, Any], seed: int, device: torch.device) -> dict[str, Any]:
    ...
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C32 formal attempt not prepared")
    ...
    if report_path.exists() or score_path.exists():
        raise FileExistsError(f"C32 seed output exists: {seed}")
    started = time.time()
    store = FrozenTransportStore(config)
    if candidate_hashes(store) != attempt["candidate_hashes"]:
        raise RuntimeError("C32 candidate hashes changed before seed training")
    labels = fit_labels(config)
    model = make_model(config, seed)
    identity = model_identity(model)
    if not identity["up_projection_initialized_exact_zero"]:
        raise RuntimeError("C32 neutral adapter initialization differs")
...
def load_seed_scores(report: Mapping[str, Any], store: FrozenTransportStore) -> dict[str, Any]:
    ...
    if sha256_file(path) != report["score_artifact"]["sha256"]:
        raise RuntimeError("C32 seed score artifact changed")
    ...
    if indices != expected:
        raise RuntimeError("C32 seed score request order differs")
...
def aggregate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C32 train-gate report exists")
    attempt_path = root / "formal_attempt.json"
    attempt = read_json(attempt_path)
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C32 aggregate attempt state differs")
    store = FrozenTransportStore(config)
    hashes = candidate_hashes(store)
    if hashes != attempt["candidate_hashes"]:
        raise RuntimeError("C32 candidate hashes changed before aggregate")
    seeds = [int(value) for value in config["training"]["seeds"]]
    ...
    for seed in seeds:
        ...
        if int(row.get("seed", -1)) != seed or row.get("internal_A_labels_opened") is not False:
            raise ValueError("C32 seed report identity/boundary differs")
        if row.get("candidate_hashes") != hashes:
            raise RuntimeError("C32 seed candidate hash differs")
    ...
    a0_checks = {
        ...
        "selection_integrity": (
            selection_checks["c32_internal_A_features_scores_labels_opened"] is False
            and selection_checks["c32_delayed_B_features_scores_labels_opened"] is False
            and selection_checks["c31_A_labels_previously_opened_but_not_reused"] is True
            ...
            and selection_checks["c32_code_dev_test_qrels_metrics_read"] is False
        ),
    }
    a0 = {
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C32 candidate hashes changed before A1 label access")
