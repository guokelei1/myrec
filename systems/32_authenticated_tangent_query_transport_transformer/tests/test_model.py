...
def test_history_write_is_orthogonal_to_the_adapted_query() -> None:
    model = make_model()
    query = F.normalize(torch.randn(8), dim=0)
    history = F.normalize(torch.randn(4, 8), dim=1)
    adapted_query, tangent, transported = model.transport_state(query, history)
    assert abs(float(torch.dot(adapted_query, tangent))) < 1e-6
    assert torch.allclose(transported.norm(), torch.tensor(1.0), atol=1e-6)
