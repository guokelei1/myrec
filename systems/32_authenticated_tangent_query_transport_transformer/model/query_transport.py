"""Authenticated spherical-tangent query transport in a shared LM space."""
...
PRIMARY = "authenticated_tangent_query_transport"
MODES = (
    PRIMARY,
    "unprojected_query_transport",
    "adapted_attention_tangent_transport",
    "unauthenticated_tangent_transport",
)


class LowRankQueryTransport(nn.Module):
    """Move a query only along the unit sphere's candidate-relative tangent."""
    ...
    def __init__(
        ...
        *,
        ...
        if min(self.dim, self.rank) <= 0 or min(
            self.temperature, self.profile_scale, self.correction_scale
        ) <= 0:
            raise ValueError("C32 dimensions/scales must be positive")
    ...
    def transport_state(
        ...
        tangent = profile - torch.dot(profile, adapted_query) * adapted_query
        transported = F.normalize(
            adapted_query + self.profile_scale * tangent, dim=-1, eps=1e-6
        )
        return adapted_query, tangent, transported
    ...
    def forward(
        ...
        if (
            ...
            raise ValueError("C32 embedding shape differs")
        if len(history) == 0:
            return torch.zeros(
...
class FrozenBGETransportRanker(nn.Module):
    ...
    def forward(
        ...
        if len(query) != 1:
            raise ValueError("C32 expects one query per request")
        history = (
