"""Create immutable C36 proposal and post-G0 execution locks."""
...
def external_paths(config: dict[str, Any]) -> list[Path]:
    ...
    if outside:
        raise ValueError(f"C36 external input outside repository: {outside}")
    return result
...
def freeze_proposal(config: dict[str, Any]) -> dict[str, Any]:
    ...
    value = {
        ...
        "declarations": {
            "c35_fit_labels_previously_opened_and_reused": True,
            ...
            "c36_fit_labels_opened_in_this_attempt": False,
            "c36_internal_A_features_labels_scores_opened": False,
            "c36_delayed_B_features_labels_scores_opened": False,
            "c36_ranking_outcome_observed": False,
        ...
        "lock_id": "c36_conservative_barycentric_transport_gate_v1",
        "selection_path": relative_repo(config["paths"]["selection"]),
        "selection_sha256": config["paths"]["selection_sha256"],
        "status": "locked_before_c36_A_feature_label_score_or_outcome",
    }
...
def freeze_execution(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if report.get("status") != "passed":
        raise PermissionError("C36 G0 is not a pass")
    if report.get("internal_A_labels_opened") is not False:
        raise PermissionError("C36 A labels opened before execution lock")
    if report.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C36 delayed-B boundary differs")
    value = {
        "candidate_id": "c36",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "declarations": {
        ...
        "lock_id": "c36_conservative_barycentric_transport_execution_v1",
