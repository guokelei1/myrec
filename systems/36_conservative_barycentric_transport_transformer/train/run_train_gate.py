"""Parallel-seed, staged C36 train-only architecture gate."""
...
def assert_cuda(config: Mapping[str, Any], seed: int, device: torch.device) -> int:
    ...
    if seed not in seeds:
        raise ValueError(f"unregistered C36 seed: {seed}")
    physical = physicals[seeds.index(seed)]
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C36 seed/GPU registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C36 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C36 seed process requires exactly one visible GPU")
    return physical
...
def model_identity(model: LowRankBarycentricTransport) -> dict[str, Any]:
    return {
        "architecture": {
            PRIMARY: "conservative_barycentric_tangent_transport",
            GLOBAL_CONTROL: "candidate_shared_global_tangent_transport",
            UNBOUNDED_CONTROL: "unbounded_barycentric_tangent_transport",
            UNCENTERED_CONTROL: "uncentered_trust_tangent_transport",
            RELATIVE_ONLY_CONTROL: "candidate_relative_surplus_only",
        }[model.mode],
        "mode": model.mode,
        "candidate_specific_transport": model.mode != GLOBAL_CONTROL,
        "conservative_barycentric_transport": model.mode == PRIMARY,
        "active_set_barycentric_centering": model.mode
        in {PRIMARY, UNBOUNDED_CONTROL},
        "request_max_norm_trust": model.mode
        in {PRIMARY, UNCENTERED_CONTROL},
        "unbounded_barycentric_transport": model.mode == UNBOUNDED_CONTROL,
        "uncentered_trust_transport": model.mode == UNCENTERED_CONTROL,
...
def training_rows(
    ...
    for index in store.role_indices("fit"):
        ...
        if stop - start != count:
            raise ValueError("C36 fit label count differs")
...
def train_primary(
    model: LowRankBarycentricTransport,
    store: FrozenTransportStore,
    ...
    if listwise_weight <= 0 or direction_weight <= 0:
        raise ValueError("C36 fixed loss weights must be positive")
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("nonfinite C36 training loss")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C36 gradient: {name}")
...
def write_npz_once(path: Path, **arrays: np.ndarray) -> None:
    if path.exists():
        raise FileExistsError(f"immutable C36 score artifact exists: {path}")
...
def candidate_hashes(store: FrozenTransportStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C36 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C36 checkpoint exists: {path}")
    temporary = path.with_suffix(path.suffix + ".tmp")
    torch.save(
        {
            "candidate_id": "c36",
...
def barycentric_diagnostics(
    ...
    with torch.inference_mode():
        for index in indices:
            ...
            coefficient = state["trust_coefficient"]
            ...
            if float(global_norm.cpu()) > 0:
                ratio = (
                    coefficient
                    * torch.linalg.vector_norm(centered, dim=1).max()
                    / global_norm
                )
                maximum_residual_to_global_ratio = max(
                    maximum_residual_to_global_ratio, float(ratio.cpu())
                )
            trust_coefficients.append(float(coefficient.cpu()))
    if not active_requests or not candidate_total:
        raise RuntimeError("C36 barycentric diagnostic surface empty")
    return {
        ...
        "maximum_bounded_residual_to_global_norm_ratio": maximum_residual_to_global_ratio,
        "trust_coefficient_min": min(trust_coefficients),
        "trust_coefficient_mean": float(np.mean(trust_coefficients)),
        "trust_coefficient_max": max(trust_coefficients),
    }
...
def prepare(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if attempt_path.exists() or report_path.exists():
        raise FileExistsError("C36 formal attempt already exists")
...
def run_seed(
    ...
    if mode not in MODES:
        raise ValueError(f"unregistered C36 mode: {mode}")
    ...
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C36 formal attempt not prepared")
    ...
    if report_path.exists() or score_path.exists():
        raise FileExistsError(f"C36 seed output exists: {seed}")
    started = time.time()
    store = FrozenTransportStore(config)
    if candidate_hashes(store) != attempt["candidate_hashes"]:
        raise RuntimeError("C36 candidate hashes changed before seed training")
    labels = fit_labels(config)
    model = make_model(config, seed, mode)
    identity = model_identity(model)
    if not identity["up_projection_initialized_exact_zero"]:
        raise RuntimeError("C36 neutral adapter initialization differs")
...
def load_seed_scores(report: Mapping[str, Any], store: FrozenTransportStore) -> dict[str, Any]:
    ...
    if sha256_file(path) != report["score_artifact"]["sha256"]:
        raise RuntimeError("C36 seed score artifact changed")
    ...
    if indices != expected:
        raise RuntimeError("C36 seed score request order differs")
...
def aggregate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C36 train-gate report exists")
    attempt_path = root / "formal_attempt.json"
    attempt = read_json(attempt_path)
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C36 aggregate attempt state differs")
    store = FrozenTransportStore(config)
    hashes = candidate_hashes(store)
    if hashes != attempt["candidate_hashes"]:
        raise RuntimeError("C36 candidate hashes changed before aggregate")
    seeds = [int(value) for value in config["training"]["seeds"]]
    ...
    for mode in MODES:
        for seed in seeds:
            ...
            if (
                ...
                raise ValueError("C36 seed report identity/boundary differs")
            if row.get("candidate_hashes") != hashes:
                raise RuntimeError("C36 seed candidate hash differs")
    ...
    a0_checks = {
        ...
        "primary_uses_conservative_barycentric_transport": all(
        ...
        "unbounded_control_removes_only_trust_bound": all(
            seed_reports[UNBOUNDED_CONTROL][seed]["model_identity"][
        ...
        "candidate_deviation_is_strictly_subordinate": all(
            float(
                seed_reports[PRIMARY][seed]["barycentric_diagnostics"][
                    "maximum_bounded_residual_to_global_norm_ratio"
                ]
            )
            < 1.0
            for seed in seeds
        ),
        "trust_bound_is_load_bearing": all(
            float(
                seed_reports[PRIMARY][seed]["barycentric_diagnostics"][
                    "trust_coefficient_mean"
                ]
            )
            < 1.0
            for seed in seeds
        ...
        "selection_integrity": (
            selection_checks["c36_internal_A_features_scores_labels_opened"] is False
            and selection_checks["c36_delayed_B_features_scores_labels_opened"] is False
            and selection_checks["c35_fit_reused_with_labels_previously_opened"] is True
            and selection_checks["c35_A_features_scores_labels_opened_but_not_reused"] is True
            and selection_checks["c35_delayed_B_promoted_to_c36_A_unopened"] is True
            and selection_checks["c35_escrow_promoted_to_c36_B_unopened"] is True
            and selection_checks["fresh_c36_escrow_and_structural_roles"] is True
            ...
            and selection_checks["c36_code_dev_test_qrels_metrics_read"] is False
        ),
    }
    a0 = {
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C36 candidate hashes changed before A1 label access")
    ...
    status = (
        "passed_A1_conservative_barycentric_transport_delayed_B_authorized"
        if a1["status"] == "passed"
