"""Run C66's label-free canonical counterfactual-state mechanics gate."""
...
from train.data_bridge import C64Store, candidate_keys, to_device  # noqa: E402
...
def merge_batches(
    true_tensors: Mapping[str, torch.Tensor],
    wrong_tensors: Mapping[str, torch.Tensor],
    keys: torch.Tensor,
    ...
    output.update(
        {
            ...
            "wrong_history_attention_mask": wrong_tensors[
...
def gradient_groups(
    ...
    return {
        ...
        "joint_transformer": any(
            name.startswith("inner.core.joint_transformer.") for name in active
        ),
        "output_head": any(
            name.startswith("inner.core.output_head.") for name in active
        ),
        "residual_norm": any(
            name.startswith("inner.residual_norm.") for name in active
        ),
...
def run(config: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C66 G0 physical GPU differs")
    if (
        str(device) != "cuda:0"
        or not torch.cuda.is_available()
        or torch.cuda.device_count() != 1
    ):
        raise RuntimeError("C66 G0 requires one registered visible GPU")
    ...
    true_batch = store.collate(
        probe_indices, label_access=False, history_source="true"
    )
    wrong_batch = store.collate(
        probe_indices, label_access=False, history_source="wrong"
    )
    true_tensors = to_device(true_batch, device)
    wrong_tensors = to_device(wrong_batch, device)
    key_tensor = torch.from_numpy(candidate_keys(true_batch)).to(device)
    values = merge_batches(true_tensors, wrong_tensors, key_tensor)
    ...
    with torch.inference_mode():
        ...
        permutation_error = float(
            (clean.scores - reversed_output.scores[:, permutation])
            .abs()
            .max()
            .cpu()
        )
        exact_permutation = torch.equal(
            clean.scores, reversed_output.scores[:, permutation]
        )
        deterministic_error = float((clean.scores - again.scores).abs().max().cpu())
        empty_values = dict(values)
        empty_values["history_event_mask"] = torch.zeros_like(
            values["history_event_mask"]
        )
    ...
    parameter_counts = {
        "hidden_residual_wrong_neutral": primary.parameter_count()
    }
    ...
    checks = {
        ...
        "stable_keys_present_and_unique": all(
            len(np.unique(row[mask])) == int(mask.sum())
            for row, mask in zip(
                key_tensor.cpu().numpy(),
                true_batch["candidate_mask"].astype(bool),
            )
        ),
        "canonicalizer_adds_no_parameters": primary.parameter_count()
        == primary.inner.parameter_count(),
        ...
        "candidate_permutation_bit_exact": exact_permutation,
