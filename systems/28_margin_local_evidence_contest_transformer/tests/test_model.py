...
from model.margin_local import MODES, MarginLocalEvidenceContestTransformer


def model(mode: str = "local_evidence_contest") -> MarginLocalEvidenceContestTransformer:
...
def test_primary_uses_history_and_candidate_control_does_not() -> None:
    ...
    primary = model("local_evidence_contest")
    assert not torch.equal(primary(**values).scores[0], primary(**changed).scores[0])
    control = model("local_candidate_contest")
...
def test_neutral_contest_preserves_base_order() -> None:
    ...
    scores, complement, diagonal = candidate.contest_scores(
        base, torch.zeros(1, 4, 4), mask, local=True
    )
    assert torch.equal(scores, base)
    assert float(complement) < 1e-7 and float(diagonal) == 0.0


def test_margin_local_readout_differs_from_uniform_without_new_parameters() -> None:
    base = torch.tensor([[2.0, 0.2, 0.0, -2.0]])
    mask = torch.ones_like(base, dtype=torch.bool)
    delta = torch.tensor(
        [[[0.0, 0.4, -0.2, 0.8], [-0.4, 0.0, 0.7, -0.1],
          [0.2, -0.7, 0.0, 0.3], [-0.8, 0.1, -0.3, 0.0]]]
    )
    local, _, _ = MarginLocalEvidenceContestTransformer.contest_scores(
        base, delta, mask, local=True
    )
    uniform, _, _ = MarginLocalEvidenceContestTransformer.contest_scores(
        base, delta, mask, local=False
    )
    assert not torch.allclose(local, uniform)


def test_margin_scale_is_frozen() -> None:
    with pytest.raises(ValueError):
        MarginLocalEvidenceContestTransformer.contest_scores(
            torch.zeros(1, 2), torch.zeros(1, 2, 2), torch.ones(1, 2, dtype=torch.bool),
            local=True, margin_kernel_scale=0.5
        )
