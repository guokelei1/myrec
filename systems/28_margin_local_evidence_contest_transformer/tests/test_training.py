...
def test_batch_budget_resets_maxima_after_yield() -> None:
    class Data:
        candidate_offsets = np.asarray([0, 100, 110, 120])
        history_offsets = np.asarray([0, 50, 51, 52])

    class Store:
        data = Data()
        feature_position = {0: 0, 1: 1, 2: 2}
        query_attention = np.asarray(
            [[1] * 10, [1, 1, 1] + [0] * 7, [1, 1, 1] + [0] * 7], dtype=bool
        )
    ...
    rows = list(
        iter_batches(
            ...
            shuffle=False,
            max_requests=8,
            max_bridge_cells=160000,
            max_pair_cells=15000,
        )
    )
    assert [row.tolist() for row in rows] == [[0], [1, 2]]
