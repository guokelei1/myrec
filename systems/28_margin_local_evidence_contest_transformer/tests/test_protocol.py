...
def test_selection_and_label_barriers_are_frozen() -> None:
    ...
    assert selection["checks"]["c27_escrow_features_or_labels_opened"] is False
    assert selection["checks"]["c28_internal_A_labels_opened"] is False
    assert selection["checks"]["c28_delayed_B_labels_opened"] is False
    assert selection["checks"]["donors_outside_outcome_roles"] is True
