"""One-shot staged C28 evidence-contest GPU gate."""
...
from model.margin_local import MODES, MarginLocalEvidenceContestTransformer  # noqa: E402
...
PRIMARY = "local_evidence_contest"
...
def assert_cuda(config: Mapping[str, Any], device: torch.device) -> None:
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(
        ...
        raise RuntimeError("C28 CUDA registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C28 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C28 requires one visible GPU")
...
def make_model(
    ...
    padding_idx: int,
) -> MarginLocalEvidenceContestTransformer:
    value = config["model"]
    return MarginLocalEvidenceContestTransformer(
        ...
        pair_delta_max=float(value["pair_delta_max"]),
        additive_delta_max=float(value["additive_delta_max"]),
        margin_kernel_scale=float(value["margin_kernel_scale"]),
        mode=mode,
    )
    ...
    *,
    ...
    return list(
        iter_batches(
            ...
            max_pair_cells=int(training["max_pair_cells"]),
        )
    )
...
def train_model(
    model: MarginLocalEvidenceContestTransformer,
    store: FrozenTokenStore,
    ...
    for epoch_batches in schedules:
        for indices in epoch_batches:
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError(f"nonfinite C28 loss: {model.mode}")
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C28 gradient: {model.mode}/{name}")
...
def score(
    ...
    *,
    ...
    complement_error = diagonal_error = 0.0
    with torch.no_grad():
        for request_batch in batches(store, indices, config, seed=0, shuffle=False):
            ...
            complement_error = max(
                complement_error, float(result.pair_complement_error.cpu())
            )
            diagonal_error = max(diagonal_error, float(result.pair_diagonal_error.cpu()))
    ...
    if rows["request_ids"] != [store.data.request_ids[int(index)] for index in indices]:
        raise ValueError("C28 score order differs")
    rows["pair_complement_max_abs_difference"] = complement_error
    rows["pair_diagonal_max_abs_difference"] = diagonal_error
    return rows
...
def neutral_order_changes(scored: Mapping[str, Any]) -> dict[str, Any]:
    neutral: list[np.ndarray] = []
    for row in scored["base_scores"]:
        base = torch.from_numpy(np.asarray(row, dtype=np.float32))[None]
        mask = torch.ones_like(base, dtype=torch.bool)
        delta = torch.zeros(1, len(row), len(row), dtype=base.dtype)
        scores, _, _ = MarginLocalEvidenceContestTransformer.contest_scores(
            base, delta, mask, local=True
        )
        neutral.append(scores[0].numpy())
    reference = {**scored, "scores": scored["base_scores"]}
    proposed = {**scored, "scores": neutral}
    return order_changes(reference, proposed)
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C28 checkpoint exists: {path}")
    state = {
        name: value
...
def candidate_hashes(store: FrozenTokenStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C28 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def formal(config: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    if report_path.exists() or attempt_path.exists():
        raise FileExistsError("C28 formal attempt exists")
    atomic_json(
        attempt_path,
        {
            "candidate_id": "c28",
    ...
    deterministic_max = permutation_max = 0.0
    pair_complement_max = pair_diagonal_max = 0.0
    ...
    for seed in seeds:
        ...
        for mode in MODES:
            ...
            if mode == PRIMARY:
                ...
                pair_complement_max = max(
                    pair_complement_max,
                    float(clean_A["pair_complement_max_abs_difference"]),
                )
                pair_diagonal_max = max(
                    pair_diagonal_max,
                    float(clean_A["pair_diagonal_max_abs_difference"]),
                )
    ...
    neutral_order = neutral_order_changes(
        outputs[seeds[0]]["internal_A"]["modes"][PRIMARY]
    )
    gate = config["gate"]
    ...
    a0_checks = {
        ...
        "pair_complement": pair_complement_max
        <= float(gate["pair_complement_max_abs_difference"]),
        "pair_diagonal": pair_diagonal_max
        <= float(gate["pair_diagonal_max_abs_difference"]),
        "neutral_preserves_base_order": neutral_order["any_count"]
        <= int(gate["neutral_order_difference_max"]),
        ...
        "wrong_changes_top10": all(
            row["top10_fraction"] >= float(gate["wrong_top10_change_fraction_min"])
            for row in wrong_order.values()
        ),
        ...
        "selection_integrity": (
            checks["c27_escrow_features_or_labels_opened"] is False
            and checks["c28_internal_A_labels_opened"] is False
            and checks["c28_delayed_B_labels_opened"] is False
            and checks["roles_pairwise_disjoint"] is True
            and checks["donors_outside_outcome_roles"] is True
    ...
    a0 = {
        ...
        "neutral_order_changes": neutral_order,
        "pair_complement_max_abs_difference": pair_complement_max,
        "pair_diagonal_max_abs_difference": pair_diagonal_max,
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C28 candidate hashes changed before A1")
    a1 = utility_gate(
        role="internal_A",
        indices=probe_indices,
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C28 candidate hashes changed before A2")
    a2 = utility_gate(
        role="delayed_B",
        indices=delayed_indices,
