"""C28 frozen selection, packed structure, and file utilities."""
...
def write_json_once(path: str | Path, value: Mapping[str, Any]) -> None:
    target = Path(path)
    if target.exists():
        raise FileExistsError(f"immutable C28 output exists: {target}")
...
def load_config(path: str | Path, *, require_selection: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("candidate_id") != "c28":
        raise ValueError("unexpected C28 config")
    if config.get("gate_id") != "c28_margin_local_evidence_contest_v1":
        raise ValueError("unexpected C28 gate")
    actual = {
    ...
    if actual != ROLE_COUNTS or int(config["resources"]["physical_gpu"]) != 0:
        raise ValueError("C28 frozen role/GPU registration differs")
...
class PackedStructure:
    def __init__(self, root: str | Path) -> None:
        ...
        if len(self.request_ids) + 1 != len(self.candidate_offsets):
            raise ValueError("C28 packed request/candidate mismatch")
...
def stable_key(seed: int, role: str, request_id: str) -> tuple[bytes, str]:
    payload = (
        b"c28-margin-local-evidence-contest-v1\0"
