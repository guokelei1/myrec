"""C26 proposal and execution lock verification."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if not path.is_file():
        raise PermissionError("C26 proposal lock missing")
    lock = read_json(path)
    if lock.get("lock_id") != "c26_query_pivot_token_bridge_v2" or lock.get(
        "status"
    ) != "locked_before_any_c26_internal_or_delayed_label_or_outcome":
        raise ValueError("unexpected C26 proposal lock")
    failures: list[str] = []
    lines: list[str] = []
    ...
    if failures:
        raise RuntimeError(f"C26 proposal lock mismatch: {failures}")
    declarations = lock["declarations"]
    if declarations.get("c26_internal_A_labels_opened") is not False or declarations.get(
        "c26_ranking_outcome_observed"
    ) is not False:
        raise ValueError("C26 lock is not pre-outcome")
...
def verify_execution_lock(
    ...
    for name, metadata in report["outputs"].items():
        ...
        if expected != metadata["sha256"] or sha256_file(metadata["path"]) != expected:
            raise RuntimeError(f"C26 G0 output changed: {name}")
    return lock, sha256_file(path)
