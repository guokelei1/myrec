...
def test_selection_and_label_barriers_are_frozen() -> None:
    ...
    assert ROLE_COUNTS["fit"] == 6000 and ROLE_COUNTS["delayed_B"] == 1200
    assert sha256_file(config["paths"]["selection"]) == config["paths"]["selection_sha256"]
    selection = read_json(config["paths"]["selection"])
    assert selection["checks"]["c26_internal_A_labels_opened"] is False
    assert selection["checks"]["c26_delayed_B_labels_opened"] is False
    ...
    assert config["paths"]["execution_lock"].endswith("execution_lock_v2.json")
