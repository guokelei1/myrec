"""Materialize frozen contextual BGE tokens for C61's fit/fresh-A surface."""
...
def donor_indices(selection: Mapping[str, Any], role: str) -> list[int]:
    return [int(value) for value in selection["wrong_history_donors"][role]["indices"]]


def required_indices(config: Mapping[str, Any]) -> tuple[np.ndarray, np.ndarray]:
    selection = read_json(REPO_ROOT / config["paths"]["c26_selection"])
    data = PackedData(REPO_ROOT / config["paths"]["packed_train_root"])
    requests = sorted(
        set(
            value
            for role in ("fit", "internal_A", "structural_nohistory", "structural_repeat")
            for value in role_indices(selection, role)
        )
    )
    donors = sorted(
        set(
            ...
            for value in donor_indices(selection, role)
        )
    )
    items: set[int] = set()
    ...
    for index in donors:
        items.update(map(int, data.history(index)))
    return np.asarray(requests, dtype=np.int64), np.asarray(sorted(items), dtype=np.int64)
...
def assert_cuda(config: Mapping[str, Any], shard: int, device_name: str) -> None:
    ...
    if device_name != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C61 contextual GPU binding differs")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C61 contextual materialization requires one visible GPU")


def seed_all(seed: int = 20264200) -> None:
    random.seed(seed)
...
def materialize_shard(
    ...
    if bool((item_positions >= len(source_items)).any()) or not np.array_equal(
        ...
        raise RuntimeError("C61 contextual item token source missing")
    ...
    if any(int(value) not in position for value in request_subset):
        raise RuntimeError("C61 contextual query token source missing")
    request_positions = np.asarray([position[int(value)] for value in request_subset], dtype=np.int64)
    item_ids = np.asarray(np.load(source_root / "item_token_ids.npy", mmap_mode="r")[item_positions], dtype=np.int32)
    item_attention = np.asarray(np.load(source_root / "item_attention_mask.npy", mmap_mode="r")[item_positions], dtype=bool)
    item_content = np.asarray(np.load(source_root / "item_content_mask.npy", mmap_mode="r")[item_positions], dtype=bool)
    query_ids = np.asarray(np.load(source_root / "query_token_ids.npy", mmap_mode="r")[request_positions], dtype=np.int32)
    query_attention = np.asarray(np.load(source_root / "query_attention_mask.npy", mmap_mode="r")[request_positions], dtype=bool)
    query_content = np.asarray(np.load(source_root / "query_content_mask.npy", mmap_mode="r")[request_positions], dtype=bool)
    ...
    if report_path.exists() or any(path.exists() for path in files.values()):
        raise FileExistsError(f"immutable C61 contextual shard exists: {shard}")
    ...
    if int(model.config.hidden_size) != hidden:
        raise RuntimeError("C61 contextual LM width differs")
    for ids, attention, output in (
        (item_ids, item_attention, files["item_states"]),
        (query_ids, query_attention, files["query_states"]),
    ):
        encode_to_memmap(
            model=model,
            ids=ids,
            attention=attention,
            output_path=output,
            batch_size=int(config["encoding"]["materialization_batch_size"]),
            hidden_dim=hidden,
    ...
    report = {
        ...
        "materialization_lock_sha256": lock_hash,
...
def aggregate(config_path: str | Path) -> dict[str, Any]:
    ...
    reports = [
        ...
        for shard in range(int(config["resources"]["materialization_shards"]))
    ]
    for report in reports:
        for output in report["outputs"].values():
            if sha256_file(REPO_ROOT / output["path"]) != output["sha256"]:
                raise RuntimeError("C61 contextual shard output changed")
    ...
    checks = {
        ...
        "fit_labels_closed_during_materialization": True,
...
def main() -> None:
    ...
    if args.stage == "shard":
        if args.shard is None:
            raise ValueError("C61 shard stage requires --shard")
