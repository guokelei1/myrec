"""Materialize C29 D2p, token, authentication, and compact fit-label inputs."""
...
def assert_cuda(config: Mapping[str, Any], device: str) -> None:
    ...
    if device != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C29 G0 GPU registration mismatch")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C29 G0 requires one visible CUDA GPU")
...
def selected_items(
    ...
    feature_set = {int(value) for value in indices}
    ...
    for role, row in selection["wrong_history_donors"].items():
        recipients = selection["roles"][role]["indices"]
        for recipient, donor in zip(recipients, row["indices"]):
...
def tokenize_items(
    *,
    ...
    with Path(corpus_path).open("r", encoding="utf-8") as handle:
        for row, line in enumerate(handle):
            ...
            if row < target:
                continue
            if row != target:
                raise ValueError(f"C29 corpus row skipped: {row}/{target}")
            value = json.loads(line)
            item_id = str(value["item_id"])
            if int(mapping[item_id]) != row:
                raise ValueError("C29 corpus/item mapping differs")
    ...
    flush()
    if pointer != len(item_indices):
        raise ValueError("C29 corpus coverage differs")
...
def materialize(config_path: str | Path, device: str) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C29 G0 exists")
    ...
    if c28_outcome.get("internal_A_labels_opened") is not True:
        raise PermissionError("C29 inherited fit label state differs")
    if c28_outcome.get("delayed_B_labels_opened") is not False:
        raise PermissionError("C29 source delayed-B label state differs")
    if c28_g0.get("escrow_features_or_labels_opened") is not False:
        raise PermissionError("C29 source escrow boundary differs")
    ...
    if len(feature_indices) != len(set(int(value) for value in feature_indices)):
        raise AssertionError("C29 feature roles overlap")
    ...
    positions = {
        int(index): row for row, index in enumerate(authentication.request_indices)
    }
    A_indices = [int(value) for value in selection["roles"]["internal_A"]["indices"]]
    A_positions = np.asarray([positions[index] for index in A_indices], dtype=np.int64)
    true_counts = np.diff(authentication.true_offsets)[A_positions]
    wrong_counts = np.diff(authentication.wrong_offsets)[A_positions]
    true_source = authentication.true_source_counts[A_positions]
    wrong_source = authentication.wrong_source_counts[A_positions]
    ...
    difference_bootstrap = bootstrap_mean(
        difference,
    ...
    auth_gate = {
        "status": "passed" if all(auth_checks.values()) else "failed",
        "checks": auth_checks,
        ...
        "true_authenticity": bootstrap_mean(
            true_fraction,
            samples=int(config["evaluation"]["bootstrap_samples"]),
            seed=int(config["selection"]["seed"]) + 1,
        ),
        "wrong_authenticity": bootstrap_mean(
            wrong_fraction,
            samples=int(config["evaluation"]["bootstrap_samples"]),
            seed=int(config["selection"]["seed"]) + 2,
        ),
    ...
    if checkpoint.get("analysis_id") != "finetuned_nonpersonalized_control_v1":
        raise ValueError("C29 D2 checkpoint identity differs")
    if int(checkpoint.get("seed", -1)) != 20260708:
        raise ValueError("C29 D2 checkpoint seed differs")
    ...
    if sha256_file(paths["internal_train_popularity"]) != config["integrity"][
        ...
        raise ValueError("C29 popularity differs")
    ...
    with torch.inference_mode():
        for start in range(0, len(feature_indices), batch_size):
            ...
            for row, raw_index in enumerate(requests):
                ...
                positions_in_items = np.searchsorted(item_indices, candidates)
                candidate_states = torch.from_numpy(item_states[positions_in_items]).to(device)
                ...
                if not np.isfinite(mixed).all():
                    raise ValueError("nonfinite C29 D2p")
