"""Strictly prequential user-memory authentication for C29 history events."""
...
class AuthenticationRows:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.true_offsets) != count + 1 or len(self.wrong_offsets) != count + 1:
            raise ValueError("C29 authentication offsets differ")
        if int(self.true_offsets[-1]) != len(self.true_items):
            raise ValueError("C29 true authentication values differ")
        if int(self.wrong_offsets[-1]) != len(self.wrong_items):
            raise ValueError("C29 wrong authentication values differ")
        for value in (self.profile_sizes, self.true_source_counts, self.wrong_source_counts):
            if len(value) != count:
                raise ValueError("C29 authentication row metadata differs")
    ...
    with Path(path).open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if users[index] is not None:
                raise ValueError("duplicate C29 label-free request metadata")
            if str(row.get("split")) != "train":
                raise ValueError("C29 user metadata contains non-train packed request")
            if int(row["time_index"]) != int(data.timestamps[index]):
                raise ValueError("C29 user metadata timestamp differs")
            users[index] = str(row["user_id"])
    if any(value is None for value in users):
        missing = sum(value is None for value in users)
        raise ValueError(f"C29 user metadata missing {missing} packed requests")
...
def build_authentication(
    *,
    ...
    if len(user_ids) != len(data.request_ids):
        raise ValueError("C29 user/request cardinality differs")
    targets = [int(value) for value in target_indices]
    if len(targets) != len(set(targets)):
        raise ValueError("C29 authentication targets overlap")
    target_set = set(targets)
    if set(donor_by_recipient) - target_set:
        raise ValueError("C29 donor mapping has non-target recipients")
    ...
    if set(true_by_index) != target_set or set(wrong_by_index) != target_set:
        raise RuntimeError("C29 authentication target coverage differs")
    true_offsets = [0]
    wrong_offsets = [0]
