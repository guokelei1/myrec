"""C29 frozen selection, packed structure, and immutable-file utilities."""
...
def write_json_once(path: str | Path, value: Mapping[str, Any]) -> None:
    target = Path(path)
    if target.exists():
        raise FileExistsError(f"immutable C29 output exists: {target}")
...
def load_config(path: str | Path, *, require_selection: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("candidate_id") != "c29":
        raise ValueError("unexpected C29 config")
    if config.get("gate_id") != "c29_causally_authenticated_mediation_v1":
        raise ValueError("unexpected C29 gate")
    actual = {
    ...
    if actual != ROLE_COUNTS or physical != [0, 1, 2]:
        raise ValueError("C29 frozen role/GPU registration differs")
    if int(config["selection"]["inherited_open_fit_requests"]) != 3_600:
        raise ValueError("C29 inherited fit registration differs")
...
class PackedStructure:
    def __init__(self, root: str | Path) -> None:
        ...
        if len(self.request_ids) + 1 != len(self.candidate_offsets):
            raise ValueError("C29 packed request/candidate mismatch")
        if len(self.request_ids) + 1 != len(self.history_offsets):
            raise ValueError("C29 packed request/history mismatch")
        if len(self.request_ids) != len(self.timestamps):
            raise ValueError("C29 packed request/timestamp mismatch")
...
def stable_key(seed: int, role: str, request_id: str) -> tuple[bytes, str]:
    payload = (
        b"c29-causally-authenticated-mediation-v1\0"
