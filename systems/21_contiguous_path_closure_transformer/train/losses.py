"""Full-candidate request-listwise loss for C21."""
...
def masked_listwise_loss(
    ...
    if scores.ndim != 2 or labels.shape != scores.shape or candidate_mask.shape != scores.shape:
        raise ValueError("C21 listwise tensors must share shape [B,C]")
    ...
    target = gains[valid] / gains[valid].sum(dim=-1, keepdim=True)
