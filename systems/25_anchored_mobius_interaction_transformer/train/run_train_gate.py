"""One-shot staged C25 GPU train-only gate."""
...
from model.amit import MODES, AnchoredMobiusInteractionTransformer  # noqa: E402
...
def assert_cuda(config: Mapping[str, Any], device: torch.device) -> None:
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(
        ...
        raise RuntimeError("C25 CUDA registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C25 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C25 requires one visible GPU")


def make_model(config: Mapping[str, Any], mode: str) -> AnchoredMobiusInteractionTransformer:
    value = config["model"]
    ...
    *,
    ...
    return list(
        iter_batches(
            ...
            max_interaction_tokens=int(training["max_interaction_tokens"]),
        )
    )
...
def train_model(
    model: AnchoredMobiusInteractionTransformer,
    store: FrozenFeatureStore,
    ...
    for epoch_batches in schedules:
        for indices in epoch_batches:
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError(f"nonfinite C25 loss: {model.mode}")
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C25 gradient: {model.mode}/{name}")
...
def score(
    ...
    *,
    ...
    if rows["request_ids"] != [store.data.request_ids[int(index)] for index in indices]:
        raise ValueError("C25 score order differs")
...
def anchored_null_audit(
    ...
    q, c, h, weights = (
        ...
        tensors["candidates"],
        tensors["history"],
        tensors["event_weights"],
    )
    with torch.no_grad():
        values = {
            "query_null": model.interaction_tokens(torch.zeros_like(q), c, h, weights),
            "candidate_null": model.interaction_tokens(q, torch.zeros_like(c), h, weights),
            "history_null": model.interaction_tokens(q, c, torch.zeros_like(h), weights),
        }
    return {name: float(value.abs().max().cpu()) for name, value in values.items()}
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C25 checkpoint exists: {path}")
    torch.save(
        {
            "candidate_id": "c25",
            "seed": seed,
...
def candidate_hashes(store: FrozenFeatureStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C25 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def utility_gate(
    *,
    ...
    return {
        ...
        "wrong_history_retention": edge_retention,
...
def formal(config: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    if report_path.exists() or attempt_path.exists():
        raise FileExistsError("C25 formal attempt exists")
    atomic_json(
        attempt_path,
        {
            "candidate_id": "c25",
    ...
    deterministic_max = centre_max = permutation_max = anchored_max = 0.0
    ...
    for seed in seeds:
        ...
        mode_outputs_A: dict[str, Any] = {}
        mode_outputs_B: dict[str, Any] = {}
        ...
        for mode in MODES:
            ...
            mode_outputs_A[mode], mode_outputs_B[mode] = clean_A, clean_B
            if mode == PRIMARY:
                ...
                wrong_A = score(
                    model, store, probe_indices, config, device, history_source="wrong"
                )
                wrong_B = score(
                    model, store, delayed_indices, config, device, history_source="wrong"
                )
                ...
                repeat = score(
                    model, store, store.role_indices("structural_repeat"), config, device
                )
                nohistory = score(
                ...
                null_values = anchored_null_audit(
                    model, store, probe_indices, config, device
                )
                anchored_max = max(anchored_max, *null_values.values())
            del model
            torch.cuda.empty_cache()
        assert wrong_A is not None and wrong_B is not None
        initial_hashes[str(seed)] = hashes_by_mode
        outputs[seed] = {
            "internal_A": {"modes": mode_outputs_A, "wrong": wrong_A},
            "delayed_B": {"modes": mode_outputs_B, "wrong": wrong_B},
        }
        summaries[str(seed)] = {
    ...
    a0_checks = {
        ...
        "candidate_permutation": permutation_max <= float(
            gate["candidate_permutation_max_abs_difference"]
        ),
        "anchored_null": anchored_max <= float(gate["anchored_null_max_abs"]),
        "order_active": aggregate_order["any_fraction"] >= float(gate["order_change_fraction_min"]),
        "top10_active": aggregate_order["top10_fraction"] >= float(
            gate["top10_change_fraction_min"]
        ),
    ...
    a0 = {
        ...
        "anchored_null_max_abs_difference": anchored_max,
    }
    common = {
        "candidate_id": "c25",
        "A0": a0,
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C25 candidate hashes changed before A1")
    a1 = utility_gate(
        role="internal_A",
        indices=probe_indices,
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C25 candidate hashes changed before A2")
    a2 = utility_gate(
        role="delayed_B",
        indices=delayed_indices,
