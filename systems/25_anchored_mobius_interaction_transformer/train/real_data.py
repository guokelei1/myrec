"""Frozen C25 features, donor histories, and staged labels."""
...
from train.structure import FEATURE_ROLES, PackedStructure, candidate_key_sha256, read_json, sha256_file
...
class CompactLabels:
    ...
    def __post_init__(self) -> None:
        if len(self.offsets) != len(self.request_indices) + 1 or int(self.offsets[-1]) != len(
            ...
            raise ValueError("C25 compact label structure differs")
    ...
    def rows(self, indices: Sequence[int], counts: Sequence[int]) -> list[np.ndarray]:
        ...
        for raw_index, raw_count in zip(indices, counts):
            ...
            if index not in positions:
                raise PermissionError(f"C25 label unavailable for request {index}")
            row = positions[index]
            start, stop = int(self.offsets[row]), int(self.offsets[row + 1])
            if stop - start != count:
                raise ValueError("C25 compact label candidate count differs")
...
def open_original_labels(
    *,
    ...
    if sha256_file(selection_path) != selection_sha256:
        raise RuntimeError("C25 selection changed before label access")
    values = np.load(path, mmap_mode="r")
    if len(values) != int(data.candidate_offsets[-1]):
        raise ValueError("C25 original label rows differ")
...
class FrozenFeatureStore:
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        if len(self.feature_position) != len(self.feature_indices):
            raise ValueError("C25 feature requests duplicate")
        if not np.all(np.diff(self.item_indices) > 0):
            raise ValueError("C25 item indices not sorted")
        self.donor_by_index: dict[int, int] = {}
        for role, row in self.selection["wrong_history_donors"].items():
            recipients = self.selection["roles"][role]["indices"]
    ...
    def _states(self, indices: np.ndarray) -> np.ndarray:
        ...
        if bool((positions >= len(self.item_indices)).any()) or not np.array_equal(
            ...
            raise ValueError("C25 item state missing")
    ...
    def collate(
        ...
        *,
        ...
        if history_source not in {"true", "wrong"}:
            raise ValueError("C25 unknown history source")
        request_indices = np.asarray(indices, dtype=np.int64)
        ...
        for row, (raw_index, source_index) in enumerate(zip(request_indices, source_indices)):
            index, source = int(raw_index), int(source_index)
            ...
            if stop - start != cc:
                raise ValueError("C25 base score count differs")
        ...
        result: dict[str, Any] = {
            ...
            "query_numpy": np.asarray(
                self.query[[self.feature_position[int(index)] for index in request_indices]],
                dtype=np.float32,
            ),
            "candidate_item_ids": candidate_ids,
            "candidate_numpy": self._states(candidate_indices),
            "history_numpy": self._states(history_indices),
...
def iter_batches(
    ...
    *,
    ...
    for raw_index in order:
        ...
        next_candidates, next_history = max(max_candidates, candidates), max(max_history, history)
        if current and (
            len(current) + 1 > max_requests
            or (len(current) + 1) * next_candidates * next_history > max_interaction_tokens
        ):
