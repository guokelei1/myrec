"""Materialize registered C25 states and open fit labels only."""
...
def assert_cuda(config: Mapping[str, Any], device: str) -> None:
    ...
    if device != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C25 GPU registration mismatch")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C25 requires one visible CUDA GPU")
...
def selected_items(
    ...
    for role, row in selection["wrong_history_donors"].items():
        if role not in {"fit", "internal_A", "delayed_B"}:
            raise ValueError("C25 unexpected donor role")
        for donor in row["indices"]:
            rows.append(np.asarray(data.history_indices(int(donor)), dtype=np.int64))
...
def alignment_audit(
    ...
    if len(keys) != len(scores) or len(set(keys)) != len(keys):
        raise ValueError("C25 alignment keys differ")
    ...
    if not np.array_equal(recovered, scores):
        raise AssertionError("C25 key recovery differs")
    mismatches = 0
    for row, raw_index in enumerate(indices):
        start, stop = int(offsets[row]), int(offsets[row + 1])
        mismatches += int(
            ranked(data.request_ids[int(raw_index)], item_rows[row], scores[start:stop])
            != ranked(data.request_ids[int(raw_index)], item_rows[row], recovered[start:stop])
        )
    if mismatches:
        raise AssertionError("C25 alignment changed ranks")
...
def materialize(config_path: str | Path, device: str) -> dict[str, Any]:
    ...
    if config["authorization"].get("cohort_materialization") is not True:
        raise PermissionError("C25 G0 not authorized")
    ...
    if report_path.exists():
        raise FileExistsError("immutable C25 G0 exists")
    ...
    if checkpoint_hash != config["integrity"]["calibration_checkpoint_sha256"]:
        raise ValueError("C25 D2 checkpoint differs")
    ...
    if checkpoint.get("analysis_id") != "finetuned_nonpersonalized_control_v1" or int(
        ...
        raise ValueError("C25 D2 checkpoint identity differs")
    ...
    if popularity_hash != config["integrity"]["internal_train_popularity_sha256"]:
        raise ValueError("C25 popularity differs")
    ...
    with torch.inference_mode():
        for start in range(0, len(feature_indices), batch_size):
            ...
            token_ids = torch.from_numpy(np.asarray(input_ids[selected_requests], dtype=np.int64)).to(
                device
            )
            ...
            for row, raw_index in enumerate(selected_requests):
                ...
                if not np.isfinite(mixed).all():
                    raise ValueError("nonfinite C25 D2p")
    ...
    report = {
        ...
        "alignment": alignment,
        "wrong_history_donor_states_materialized_label_free": True,
        "base": {
