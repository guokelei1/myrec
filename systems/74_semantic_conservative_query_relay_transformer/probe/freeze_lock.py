"""Freeze C74 before any trained-model outcome."""
...
def main() -> None:
    ...
    external = (
        config["paths"]["c73_generator"],
        config["paths"]["c73_proposal_lock"],
    )
    value = {
        "candidate_id": "c74",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "status": "locked_before_any_c74_trained_model_outcome",
        ...
        "external_files": {
            relative: sha256_file(REPO_ROOT / relative) for relative in external
        },
        "declarations": {
            "trained_c74_model_outcome_observed": False,
            "formal_generator_seed_previously_used": False,
