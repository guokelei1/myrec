"""C64 source/data hash and write-once guards."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    design = {
        "config": SYSTEM_ROOT / "configs/kuai_probe.yaml",
    ...
    for name, source in design.items():
        if sha256_file(source) != lock["design_sha256"][name]:
            raise RuntimeError(f"C64 proposal source changed: {name}")
    predecessor = {
        "c53_report": REPO_ROOT / config["paths"]["c53_report"],
        "c63_report": REPO_ROOT / config["paths"]["c63_report"],
    }
    for name, source in predecessor.items():
        if sha256_file(source) != lock["predecessor_sha256"][name]:
            raise RuntimeError(f"C64 predecessor changed: {name}")
    selection = REPO_ROOT / config["paths"]["c26_selection"]
    if sha256_file(selection) != lock["selection_sha256"]["c26"]:
        raise RuntimeError("C64 C26 selection changed")
...
def verify_g0_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if lock["proposal_lock_sha256"] != proposal_hash:
        raise RuntimeError("C64 G0 lock points to another proposal")
    for relative, expected in lock["source_sha256"].items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise RuntimeError(f"C64 G0 source changed: {relative}")
    return lock, sha256_file(path)
...
def verify_execution_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if lock["g0_lock_sha256"] != g0_hash:
        raise RuntimeError("C64 execution lock points to another G0")
    for relative, expected in lock["source_sha256"].items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise RuntimeError(f"C64 execution source changed: {relative}")
    for relative, expected in lock["data_sha256"].items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise RuntimeError(f"C64 registered data changed: {relative}")
    return lock, sha256_file(path)
