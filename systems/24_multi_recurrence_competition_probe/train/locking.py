"""C24 proposal and execution lock verification."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if not path.is_file():
        raise PermissionError("C24 proposal lock missing")
    lock = read_json(path)
    if lock.get("lock_id") != "c24_multi_recurrence_competition_v2" or lock.get(
        "status"
    ) != "locked_before_any_c24_delayed_label_or_outcome":
        raise ValueError("unexpected C24 proposal lock")
    failures: list[str] = []
    lines: list[str] = []
    for relative, expected in sorted(lock["files_sha256"].items()):
        path_value = SYSTEM_ROOT / relative
    ...
    if failures:
        raise RuntimeError(f"C24 proposal lock mismatch: {failures}")
    declarations = lock["declarations"]
    if declarations.get("c24_internal_A_labels_opened") is not False or declarations.get(
        "c24_ranking_outcome_observed"
    ) is not False:
        raise ValueError("C24 lock is not pre-outcome")
...
def verify_execution_lock(
    ...
    for name, metadata in report["outputs"].items():
        ...
        if expected != metadata["sha256"] or sha256_file(metadata["path"]) != expected:
            raise RuntimeError(f"C24 G0 output changed: {name}")
    return lock, sha256_file(path)
