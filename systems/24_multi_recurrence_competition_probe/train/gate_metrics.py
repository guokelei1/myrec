...
def bootstrap(values: np.ndarray, *, samples: int, seed: int) -> dict[str, Any]:
    ...
    if values.ndim != 1 or not len(values) or not np.isfinite(values).all():
        raise ValueError("C24 bootstrap values invalid")
        ...
        for fold in range(folds):
            selected = np.asarray(
                [
                    int.from_bytes(
                        hashlib.sha256(
                            f"c24-fold:{seed}:{request_id}".encode()
                        ).digest()[:8],
                        "big",
                    )
                    % folds
...
def retention(
    clean: np.ndarray, corrupt: np.ndarray, *, samples: int, seed: int
) -> dict[str, Any]:
...
def clicked_direction(
    ...
    if not output:
        raise ValueError("C24 clicked-direction surface empty")
