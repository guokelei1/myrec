"""Freeze C37 roles from C36's untouched reserves plus a fresh escrow."""
...
def materialize(config_path: str | Path) -> dict:
    ...
    for name, expected in (
        ("c36_selection", "c36_selection_sha256"),
        ("c36_g0_report", "c36_g0_report_sha256"),
        ("c36_train_report", "c36_train_report_sha256"),
        ...
        if sha256_file(paths[name]) != paths[expected]:
            raise RuntimeError(f"C37 source changed: {name}")

    c36_selection = read_json(paths["c36_selection"])
    ...
    c36_g0 = read_json(paths["c36_g0_report"])
    c36 = read_json(paths["c36_train_report"])
    if c36.get("status") != "failed_A0_terminal" or c36.get(
        "internal_A_labels_opened"
    ) is not False:
        raise PermissionError("C37 C36 terminal state differs")
    if c36_g0.get("delayed_B_features_labels_scores_opened") is not False or c36.get(
        "delayed_B_features_labels_scores_opened"
    ) is not False:
        raise PermissionError("C37 C36 delayed-B boundary differs")
    if c36.get("escrow_dev_test_opened") is not False:
        raise PermissionError("C37 C36 escrow/dev/test boundary differs")
    ...
    prior_targets = (
        ...
        | target_roles(c36_selection)
    )
    c36_roles = {
        role: [int(value) for value in row["indices"]]
        for role, row in c36_selection["roles"].items()
    }
    roles = {
        "fit": c36_roles["fit"],
        "internal_A": c36_roles["delayed_B"],
        "delayed_B": c36_roles["escrow"],
    }
    ...
    def take(pool: list[int], role: str, count: int, excluded: set[int]) -> list[int]:
        ...
        if len(ordered) < count:
            raise RuntimeError(f"C37 insufficient label-free pool for {role}")
    ...
    if {name: len(values) for name, values in roles.items()} != ROLE_COUNTS:
        raise AssertionError("C37 role counts differ")
    flat = [value for values in roles.values() for value in values]
    if len(flat) != len(set(flat)):
        raise AssertionError("C37 roles overlap")
    ...
    def donor_for(recipient: int) -> int:
        ...
        for offset in range(len(candidates)):
            ...
            if users[donor] != users[recipient] and recipient_candidates.isdisjoint(
                ...
                return donor
        raise RuntimeError("C37 donor unavailable")
    ...
    if any(donor in outcome or donor in prior_targets for values in donors.values() for donor in values):
        raise AssertionError("C37 donor intersects a target role")
    for role, values in donors.items():
        for recipient, donor in zip(roles[role], values):
            if users[recipient] == users[donor] or not set(
                ...
                raise AssertionError("C37 donor contract differs")

    result = {
        "candidate_id": "c37",
        "selection_id": "c37_barycentric_residual_transport_selection_v1",
        "status": "frozen_before_any_c37_A_feature_score_or_label",
        "seed": seed,
        "roles": {
            role: {
                "indices": values,
        ...
        "sources": {
            "c36_selection_sha256": paths["c36_selection_sha256"],
            "c36_g0_report_sha256": paths["c36_g0_report_sha256"],
            "c36_train_report_sha256": paths["c36_train_report_sha256"],
        ...
        "checks": {
            "c36_fit_reused_with_labels_previously_opened": True,
            "c36_A_features_scores_opened_but_labels_closed_and_not_reused": True,
            "c36_delayed_B_promoted_to_c37_A_unopened": True,
            "c36_escrow_promoted_to_c37_B_unopened": True,
            "c37_internal_A_features_scores_labels_opened": False,
            "c37_delayed_B_features_scores_labels_opened": False,
            "roles_pairwise_disjoint": True,
            "strict_nonrepeat_fit_A_B_escrow": True,
            "fresh_c37_escrow_and_structural_roles": True,
