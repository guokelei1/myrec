...
def make(mode: str, seed: int = 11) -> LowRankBarycentricResidualTransport:
...
def test_primary_preserves_global_barycenter_and_inactive_state() -> None:
    ...
    assert bool(admitted.any()) and bool((~admitted).any())
...
def test_primary_natural_residual_is_subordinate_on_fixture() -> None:
    query, history, candidates = fixture()
    state = make(PRIMARY).transport_components(query, history, candidates)
    global_norm = torch.linalg.vector_norm(state["global_write"])
    residual_max = torch.linalg.vector_norm(state["centered_relative"], dim=1).max()
    assert residual_max < global_norm
    assert bool((state["displacement"].mv(state["global_write"]) > 0).all())


def test_controls_remove_barycenter_or_global_anchor() -> None:
    query, history, candidates = fixture()
    ...
    assert not torch.equal(primary["displacement"], global_only["displacement"])
