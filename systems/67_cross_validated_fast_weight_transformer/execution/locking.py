"""C67 source-hash and write-once guards."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    for name, source in design.items():
        if sha256_file(source) != lock["design_sha256"][name]:
            raise RuntimeError(f"C67 proposal source changed: {name}")
    predecessor = REPO_ROOT / config["paths"]["c66_report"]
    if sha256_file(predecessor) != lock["predecessor_sha256"]["c66_report"]:
        raise RuntimeError("C67 C66 report changed")
...
def verify_g0_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if lock["proposal_lock_sha256"] != proposal_hash:
        raise RuntimeError("C67 G0 lock points to another proposal")
    for relative, expected in lock["source_sha256"].items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise RuntimeError(f"C67 G0 source changed: {relative}")
    return lock, sha256_file(path)
