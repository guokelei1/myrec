"""C43 frozen selection, packed structure, and immutable-file utilities."""
...
def write_json_once(path: str | Path, value: Mapping[str, Any]) -> None:
    target = Path(path)
    if target.exists():
        raise FileExistsError(f"immutable C43 output exists: {target}")
...
def load_config(path: str | Path, *, require_selection: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("candidate_id") != "c43":
        raise ValueError("unexpected C43 config")
    if config.get("gate_id") != "c43_cross_domain_metric_coupled_v1":
        raise ValueError("unexpected C43 gate")
    actual = {
    ...
    if actual != ROLE_COUNTS or physical != [0, 1, 2]:
        raise ValueError("C43 frozen role/GPU registration differs")
    if config["model"]["modes"] != [
        ...
        raise ValueError("C43 frozen mode registration differs")
    if [int(value) for value in config["training"]["seeds"]] != [
        20262701,
        20262702,
        20262703,
    ]:
        raise ValueError("C43 frozen seed registration differs")
...
class PackedStructure:
    def __init__(self, root: str | Path) -> None:
        ...
        if len(self.request_ids) + 1 != len(self.candidate_offsets):
            raise ValueError("C43 packed request/candidate mismatch")
        if len(self.request_ids) + 1 != len(self.history_offsets):
            raise ValueError("C43 packed request/history mismatch")
        if len(self.request_ids) != len(self.timestamps):
            raise ValueError("C43 packed request/timestamp mismatch")
...
def stable_key(seed: int, role: str, request_id: str) -> tuple[bytes, str]:
    payload = (
        b"c43-cross-domain-metric-coupled-v1\0"
