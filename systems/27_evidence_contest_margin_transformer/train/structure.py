"""C27 frozen selection, packed structure, and file utilities."""
...
def write_json_once(path: str | Path, value: Mapping[str, Any]) -> None:
    target = Path(path)
    if target.exists():
        raise FileExistsError(f"immutable C27 output exists: {target}")
...
def load_config(path: str | Path, *, require_selection: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("candidate_id") != "c27":
        raise ValueError("unexpected C27 config")
    if config.get("gate_id") != "c27_evidence_contest_margin_v1":
        raise ValueError("unexpected C27 gate")
    actual = {
    ...
    if actual != ROLE_COUNTS or int(config["resources"]["physical_gpu"]) != 3:
        raise ValueError("C27 frozen role/GPU registration differs")
...
class PackedStructure:
    def __init__(self, root: str | Path) -> None:
        ...
        if len(self.request_ids) + 1 != len(self.candidate_offsets):
            raise ValueError("C27 packed request/candidate mismatch")
...
def stable_key(seed: int, role: str, request_id: str) -> tuple[bytes, str]:
    payload = (
        b"c27-evidence-contest-margin-v1\0"
