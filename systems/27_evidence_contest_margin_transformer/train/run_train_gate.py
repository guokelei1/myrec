"""One-shot staged C27 evidence-contest GPU gate."""
...
from model.contest_margin import MODES, EvidenceContestMarginTransformer  # noqa: E402
...
def assert_cuda(config: Mapping[str, Any], device: torch.device) -> None:
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(
        ...
        raise RuntimeError("C27 CUDA registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C27 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C27 requires one visible GPU")
...
def make_model(
    ...
    padding_idx: int,
) -> EvidenceContestMarginTransformer:
    value = config["model"]
...
def train_model(
    model: EvidenceContestMarginTransformer,
    store: FrozenTokenStore,
    ...
    for epoch_batches in schedules:
        for indices in epoch_batches:
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError(f"nonfinite C27 loss: {model.mode}")
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C27 gradient: {model.mode}/{name}")
...
def score(
    ...
    *,
    ...
    if rows["request_ids"] != [store.data.request_ids[int(index)] for index in indices]:
        raise ValueError("C27 score order differs")
...
def neutral_order_changes(scored: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in scored["base_scores"]:
        ...
        scores, _, _ = EvidenceContestMarginTransformer.contest_scores(base, delta, mask)
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C27 checkpoint exists: {path}")
    state = {
        name: value
    ...
    torch.save(
        {
            ...
            "frozen_word_embeddings_sha256": sha256_file(
                Path(config["paths"]["c26_artifact_root"]) / "word_embeddings.npy"
            ),
...
def candidate_hashes(store: FrozenTokenStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C27 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def formal(config: Mapping[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    if report_path.exists() or attempt_path.exists():
        raise FileExistsError("C27 formal attempt exists")
    atomic_json(
        attempt_path,
        {
            "candidate_id": "c27",
    ...
    a0_checks = {
        ...
        "selection_integrity": (
            checks["c26_internal_A_labels_opened"] is False
            and checks["c26_delayed_B_labels_opened"] is False
            and checks["c27_internal_A_labels_opened"] is False
            and checks["c27_delayed_B_labels_opened"] is False
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C27 candidate hashes changed before A1")
    a1 = utility_gate(
        role="internal_A",
        indices=probe_indices,
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C27 candidate hashes changed before A2")
    a2 = utility_gate(
        role="delayed_B",
        indices=delayed_indices,
