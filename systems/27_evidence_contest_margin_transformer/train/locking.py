"""C27 proposal and execution lock verification."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if not path.is_file():
        raise PermissionError("C27 proposal lock missing")
    lock = read_json(path)
    if lock.get("lock_id") != "c27_evidence_contest_margin_v1" or lock.get(
        "status"
    ) != "locked_before_any_c27_internal_or_delayed_label_or_outcome":
        raise ValueError("unexpected C27 proposal lock")
    failures: list[str] = []
    lines: list[str] = []
    ...
    if failures:
        raise RuntimeError(f"C27 proposal lock mismatch: {failures}")
    declarations = lock["declarations"]
    if declarations.get("c27_internal_A_labels_opened") is not False or declarations.get(
        "c27_ranking_outcome_observed"
    ) is not False:
        raise ValueError("C27 lock is not pre-outcome")
...
def verify_execution_lock(
    ...
    if not path.is_file():
        raise PermissionError("C27 execution lock missing")
    lock = read_json(path)
    if lock.get("status") != "locked_after_G0_before_training_or_internal_A_access":
        raise ValueError("unexpected C27 execution lock")
    if lock.get("proposal_lock_sha256") != proposal_hash:
        raise ValueError("C27 execution lock proposal mismatch")
    report_path = Path(config["paths"]["artifact_root"]) / "g0_report.json"
    if sha256_file(report_path) != lock["g0_report_sha256"]:
        raise RuntimeError("C27 G0 report changed")
    report = read_json(report_path)
    for group in ("outputs", "bound_inputs"):
        for name, metadata in report[group].items():
            expected = lock[f"g0_{group}_sha256"].get(name)
            if expected != metadata["sha256"] or sha256_file(metadata["path"]) != expected:
                raise RuntimeError(f"C27 G0 {group} changed: {name}")
    return lock, sha256_file(path)
