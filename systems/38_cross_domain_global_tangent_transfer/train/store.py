"""Frozen C38 embedding store and role-scoped train-label opener."""
...
class CompactLabels:
    ...
    def __post_init__(self) -> None:
        if len(self.offsets) != len(self.request_indices) + 1:
            raise ValueError("C38 compact label offsets differ")
        if int(self.offsets[-1]) != len(self.values):
            raise ValueError("C38 compact label values differ")
    ...
    def row(self, index: int, expected_count: int) -> np.ndarray:
        ...
        if position is None:
            raise PermissionError(f"C38 labels unavailable for record {index}")
        start, stop = int(self.offsets[position]), int(self.offsets[position + 1])
        if stop - start != expected_count:
            raise ValueError("C38 compact label candidate count differs")
...
class FrozenTransferStore:
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        if not embedding_manifest.get("finite", False):
            raise RuntimeError("C38 frozen embeddings are not finite")
        ...
        if len(self.feature_position) != len(self.feature_indices):
            raise ValueError("C38 feature request indices overlap")
    ...
    def role_indices(self, role: str) -> list[int]:
        ...
        if any(index not in self.feature_position for index in indices):
            raise PermissionError(f"C38 role {role} has no opened features")
        return indices
    ...
    def row_position(self, index: int) -> int:
        try:
            ...
            raise PermissionError(f"C38 feature unavailable for record {index}") from exc
    ...
    def history_positions(self, index: int, source: str) -> np.ndarray:
        ...
        if source == "true":
            ...
            raise ValueError(f"unknown C38 history source: {source}")
...
def open_role_labels(
    *,
    ...
    if sha256_file(records_train_path) != records_train_sha256:
        raise RuntimeError("C38 label-bearing train records changed")
    if sha256_file(selection_path) != selection_sha256:
        raise RuntimeError("C38 selection changed before label access")
    ...
    with path.open("r", encoding="utf-8") as handle:
        for expected, line in enumerate(handle):
            ...
            if int(row["position"]) != expected:
                raise ValueError(f"C38 {path.name} positions differ")
