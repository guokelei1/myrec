"""Verify C38 proposal and post-G0 execution locks."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    lock = read_json(path)
    if lock.get("lock_id") != "c38_cross_domain_global_tangent_transfer_v1":
        raise ValueError("unexpected C38 proposal lock")
    if lock.get("status") != "locked_before_fit_labels_training_or_internal_A_scores":
        raise ValueError("C38 proposal lock stage differs")
    failures = []
    lines = []
...
def verify_execution_lock(
    ...
    if not path.is_file():
        raise PermissionError("C38 execution lock missing")
    lock = read_json(path)
    if lock.get("lock_id") != "c38_cross_domain_global_tangent_execution_v1":
        raise ValueError("unexpected C38 execution lock")
    if lock.get("status") != "locked_after_G0_before_training_or_internal_A_scores":
        raise ValueError("C38 execution lock stage differs")
    if lock.get("proposal_lock_sha256") != proposal_hash:
        raise ValueError("C38 execution lock proposal differs")
    failures = []
    for relative, expected in sorted(lock["external_inputs_sha256"].items()):
        source = REPO_ROOT / relative
        if not source.is_file() or sha256_file(source) != expected:
    ...
    if failures:
        raise RuntimeError(f"C38 execution inputs changed: {failures}")
    return lock, sha256_file(path)
