"""Run the frozen C38 Amazon-C4 train-internal transfer gate."""
...
def make_model(config: Mapping[str, Any], seed: int, mode: str) -> LowRankGlobalTangentTransfer:
    model = config["model"]
...
def model_identity(model: LowRankGlobalTangentTransfer) -> dict[str, Any]:
    return {
        "mode": model.mode,
        "architecture": model.mode,
        "parameters": model.trainable_parameter_count(),
        "expected_parameters": 2 * model.dim * model.rank,
        "candidate_shared_transport": True,
        "candidate_specific_parameters_present": False,
        "candidate_scalar_head_present": False,
        "query_attended": model.mode != MEAN_TANGENT,
        "tangent_projection": model.mode != UNPROJECTED,
        "up_projection_initialized_exact_zero": bool(
            torch.equal(model.up.weight, torch.zeros_like(model.up.weight))
        ),
...
def run_g0(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    histories_nonempty = all(bool(records[index]["history"]) for index in selected)
    ...
    checks = {
        ...
        "selected_histories_nonempty": histories_nonempty,
        "selected_history_timestamps_causal": timestamps_causal,
        "wrong_donor_full_coverage": wrong["coverage_fraction"] == 1.0,
        "wrong_donor_same_length_bin_at_least_95pct": wrong[
            "same_length_bin_fraction"
        ]
        >= 0.95,
        "wrong_donor_zero_same_user": wrong["same_user_assignments"] == 0,
        "fit_has_exactly_one_positive": bool(
            all(
...
def load_fit_labels(config: Mapping[str, Any]) -> CompactLabels:
    ...
    if sha256_file(path) != g0["fit_labels"]["sha256"]:
        raise RuntimeError("C38 fit labels changed")
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("nonfinite C38 training loss")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C38 gradient: {name}")
    ...
    with torch.inference_mode():
        for index in indices:
            ...
            if store.has_repeat(index):
                ...
                correction = model(query, history, candidates).cpu().numpy().astype(np.float32)
...
def run_seed(
    ...
    if report_path.exists() or score_path.exists():
        raise FileExistsError(f"C38 seed output exists: {seed}")
    started = time.time()
    mode_reports = {}
    ...
    for mode in MODES:
        ...
        true_scores, true_corrections = score_mode(
            model, store, a_indices, "true", device
        )
        wrong_scores, _ = score_mode(model, store, a_indices, "wrong", device)
        repeat_correction_max = max(
            (
                float(np.max(np.abs(correction)))
                for index, correction in zip(a_indices, true_corrections)
                if store.has_repeat(index)
            ),
            default=0.0,
        )
        repeated_scores, _ = score_mode(model, store, a_indices[:32], "true", device)
        deterministic_scores, _ = score_mode(model, store, a_indices[:32], "true", device)
        deterministic_error = _max_difference(repeated_scores, deterministic_scores)
        ...
        with torch.inference_mode():
            for index in a_indices[:32]:
                ...
                query, history, candidates = transport_inputs(store, index, "true", device)
                reference = model(query, history, candidates)[
                    torch.from_numpy(permutation).to(device)
                ]
                _, _, permuted_candidates = transport_inputs(
                    store,
                    index,
                    "true",
                    device,
                ...
                empty = model(query, history[:0], candidates)
                nohistory_error = max(nohistory_error, float(empty.abs().max().cpu()))
                query_absent = model(
                    query,
                    history,
                    candidates,
                ...
                query_absent_error = max(
                    query_absent_error,
                    float(query_absent.abs().max().cpu()),
                )
                if mode == PRIMARY:
                    ...
                    tangent_error = max(
                        ...
                        float(
                            torch.dot(
                                state["adapted_query"], state["displacement"]
                            )
                            .abs()
                            .cpu()
                        ),
        ...
        mode_reports[mode] = {
            ...
            "tangent_orthogonality_max_abs": tangent_error if mode == PRIMARY else None,
        }
        true_rows_by_mode[mode] = true_scores
        wrong_rows_by_mode[mode] = wrong_scores
        correction_rows_by_mode[mode] = true_corrections
...
def aggregate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for seed, report in zip(seeds, reports):
        ...
        if sha256_file(score_path) != report["score_artifact"]["sha256"]:
            raise RuntimeError(f"C38 score artifact changed for seed {seed}")
        with np.load(score_path, allow_pickle=False) as values:
            offsets = np.asarray(values["offsets"], dtype=np.int64)
            rows = {
                "base": _unflatten(offsets, values["base"]),
            }
            for mode in MODES:
                rows[f"{mode}_true"] = _unflatten(offsets, values[f"{mode}_true"])
                rows[f"{mode}_wrong"] = _unflatten(offsets, values[f"{mode}_wrong"])
    ...
    activity = {
        ...
        "wrong_vs_base": order_changes(
            request_ids, item_ids, averaged["base"], averaged[f"{PRIMARY}_wrong"]
        ),
        "primary_vs_unprojected": order_changes(
            request_ids,
            item_ids,
            averaged[f"{UNPROJECTED}_true"],
            averaged[f"{PRIMARY}_true"],
        ),
        "primary_vs_mean": order_changes(
            request_ids,
            item_ids,
            averaged[f"{MEAN_TANGENT}_true"],
            averaged[f"{PRIMARY}_true"],
        ),
    }
    ...
    a0_checks = {
        ...
        "gradients_active": all(
            report["mode_reports"][mode]["training"]["nonzero_gradient_parameter_count"] > 0
            for report in reports
            for mode in MODES
        ),
        ...
        "candidate_permutation": all(
            report["mode_reports"][mode]["candidate_permutation_max_abs_difference"]
        ...
        "primary_tangent_orthogonality": all(
            report["mode_reports"][PRIMARY]["tangent_orthogonality_max_abs"] <= 1e-6
            for report in reports
        ),
        "primary_order_active": activity["primary_vs_base"]["any_fraction"] >= 0.05,
        "primary_top10_active": activity["primary_vs_base"]["top10_fraction"] >= 0.01,
        "wrong_order_active": activity["wrong_vs_base"]["any_fraction"] >= 0.02,
        "wrong_top10_active": activity["wrong_vs_base"]["top10_fraction"] >= 0.005,
        "unprojected_reduction_order_distinct": activity["primary_vs_unprojected"][
            "any_fraction"
        ]
        >= 0.02,
        "unprojected_reduction_top10_distinct": activity["primary_vs_unprojected"][
            "top10_fraction"
        ]
        >= 0.005,
        "mean_reduction_order_distinct": activity["primary_vs_mean"]["any_fraction"]
        >= 0.02,
        "mean_reduction_top10_distinct": activity["primary_vs_mean"]["top10_fraction"]
        >= 0.005,
        "no_candidate_scalar_head": all(
            not report["mode_reports"][mode]["identity"]["candidate_scalar_head_present"]
            for report in reports
            for mode in MODES
        ),
    ...
    for seed in seeds:
        per_seed_ndcg[seed] = {
            "base": ndcg_rows(request_ids, item_ids, score_rows[seed]["base"], label_rows),
        }
        for mode in MODES:
    ...
    comparisons = compare(
        ...
        averaged_ndcg[PRIMARY],
        {
            ...
            UNPROJECTED: averaged_ndcg[UNPROJECTED],
            MEAN_TANGENT: averaged_ndcg[MEAN_TANGENT],
    ...
    direction_values = clicked_direction(
    ...
    seed_differences = {
        reference: {
            str(seed): float(
                (per_seed_ndcg[seed][PRIMARY] - per_seed_ndcg[seed][reference]).mean()
            )
            for seed in seeds
        }
        for reference in ("base", UNPROJECTED, MEAN_TANGENT)
    }
    a1_checks = {
        "over_base_effect": comparisons["base"]["mean"] >= float(gate["primary_minus_base_min"]),
        "over_base_all_seeds": all(value > 0 for value in seed_differences["base"].values()),
        "over_base_all_folds": all(
            row["mean_difference"] > 0 for row in comparisons["base"]["hash_folds"]
        ),
        "over_base_ci": comparisons["base"]["percentile_95_ci"][0] > 0,
        "over_unprojected_effect": comparisons[UNPROJECTED]["mean"]
        >= float(gate["primary_minus_reduction_min"]),
        "over_unprojected_all_seeds": all(
            value >= 0 for value in seed_differences[UNPROJECTED].values()
        ),
        "over_unprojected_ci": comparisons[UNPROJECTED]["percentile_95_ci"][0] > 0,
        "over_mean_effect": comparisons[MEAN_TANGENT]["mean"]
        >= float(gate["primary_minus_reduction_min"]),
        "over_mean_all_seeds": all(
            value >= 0 for value in seed_differences[MEAN_TANGENT].values()
        ),
        "over_mean_ci": comparisons[MEAN_TANGENT]["percentile_95_ci"][0] > 0,
    ...
    report["status"] = (
        "passed_A1_cross_domain_global_tangent_transfer"
...
def _average_rows(collections: Sequence[Sequence[np.ndarray]]) -> list[np.ndarray]:
    return [np.mean(np.stack(rows), axis=0) for rows in zip(*collections)]


def _max_difference(first: Sequence[np.ndarray], second: Sequence[np.ndarray]) -> float:
...
def main() -> None:
    ...
    if args.stage == "g0":
        ...
        if args.seed is None or args.seed not in [int(value) for value in config["training"]["seeds"]]:
            raise ValueError("C38 seed is not registered")
        if not torch.cuda.is_available():
            raise RuntimeError("C38 seed stage requires CUDA")
