"""Materialize label-free C33-A features and compact fit labels after proposal lock."""
...
def assert_cuda(config: Mapping[str, Any], device: str) -> None:
    ...
    if device != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C33 G0 GPU registration mismatch")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C33 G0 requires exactly one visible CUDA GPU")
...
def validate_registered_sources(config: Mapping[str, Any]) -> None:
    ...
    for name, expected_name in (
        ...
        if sha256_file(paths[name]) != paths[expected_name]:
            raise ValueError(f"C33 registered source changed: {name}")
    c32_g0 = read_json(paths["c32_g0_report"])
    c32_outcome = read_json(paths["c32_train_report"])
    if c32_g0.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C33 source reserved role was materialized")
    if c32_outcome.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C33 source reserved boundary differs")
    if c32_outcome.get("status") != "failed_A1_terminal":
        raise PermissionError("C33 C32 terminal state differs")
    if c32_outcome.get("internal_A_labels_opened") is not True:
        raise PermissionError("C33 C32 diagnostic cohort state differs")
...
def materialize(config_path: str | Path, device: str) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C33 G0 exists")
    ...
    if len(feature_indices) != len(set(int(value) for value in feature_indices)):
        raise AssertionError("C33 feature roles overlap")
    ...
    if checkpoint.get("analysis_id") != "finetuned_nonpersonalized_control_v1":
        raise ValueError("C33 D2 checkpoint identity differs")
    if int(checkpoint.get("seed", -1)) != 20260708:
        raise ValueError("C33 D2 checkpoint seed differs")
    model = build_model(d2, device)
    model.load_state_dict(checkpoint["model_state"], strict=True)
    model.eval()
    # D2p keeps its registered fine-tuned query tower.  C33 transport must use
    ...
    if sha256_file(paths["internal_train_popularity"]) != config["integrity"][
        ...
        raise ValueError("C33 popularity differs")
        ...
        for start in range(0, len(feature_indices), batch_size):
            ...
            for row, raw_index in enumerate(requests):
                ...
                if not np.array_equal(item_indices[positions], candidates):
                    raise RuntimeError("C33 selected item lookup differs")
                ...
                if not np.isfinite(mixed).all():
                    raise ValueError("nonfinite C33 D2p")
