"""Parallel-seed, staged C33 train-only architecture gate."""
...
from model.query_transport import CONTROL, MODES, PRIMARY, LowRankQueryTransport  # noqa: E402
...
def assert_cuda(config: Mapping[str, Any], seed: int, device: torch.device) -> int:
    ...
    if seed not in seeds:
        raise ValueError(f"unregistered C33 seed: {seed}")
    physical = physicals[seeds.index(seed)]
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C33 seed/GPU registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C33 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C33 seed process requires exactly one visible GPU")
    return physical
...
def model_identity(model: LowRankQueryTransport) -> dict[str, Any]:
    return {
        "architecture": (
            "shared_low_rank_spherical_tangent_query_transport"
            if model.uses_tangent_projection
            else "shared_low_rank_unprojected_query_transport"
        ),
        "mode": model.mode,
        "uses_tangent_projection": model.uses_tangent_projection,
...
def training_rows(
    ...
    for index in store.role_indices("fit"):
        ...
        if stop - start != count:
            raise ValueError("C33 fit label count differs")
...
def train_primary(
    ...
    if listwise_weight <= 0 or direction_weight <= 0:
        raise ValueError("C33 fixed loss weights must be positive")
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("nonfinite C33 training loss")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C33 gradient: {name}")
...
def write_npz_once(path: Path, **arrays: np.ndarray) -> None:
    if path.exists():
        raise FileExistsError(f"immutable C33 score artifact exists: {path}")
...
def candidate_hashes(store: FrozenTransportStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C33 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C33 checkpoint exists: {path}")
    temporary = path.with_suffix(path.suffix + ".tmp")
    torch.save(
        {
            "candidate_id": "c33",
...
def prepare(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if attempt_path.exists() or report_path.exists():
        raise FileExistsError("C33 formal attempt already exists")
...
def run_seed(
    ...
    if mode not in MODES:
        raise ValueError(f"unregistered C33 mode: {mode}")
    ...
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C33 formal attempt not prepared")
    ...
    if report_path.exists() or score_path.exists():
        raise FileExistsError(f"C33 seed output exists: {seed}")
    started = time.time()
    store = FrozenTransportStore(config)
    if candidate_hashes(store) != attempt["candidate_hashes"]:
        raise RuntimeError("C33 candidate hashes changed before seed training")
    labels = fit_labels(config)
    model = make_model(config, seed, mode)
    identity = model_identity(model)
    if not identity["up_projection_initialized_exact_zero"]:
        raise RuntimeError("C33 neutral adapter initialization differs")
...
def load_seed_scores(report: Mapping[str, Any], store: FrozenTransportStore) -> dict[str, Any]:
    ...
    if sha256_file(path) != report["score_artifact"]["sha256"]:
        raise RuntimeError("C33 seed score artifact changed")
    ...
    if indices != expected:
        raise RuntimeError("C33 seed score request order differs")
...
def utility_gate(
    *,
    ...
    for seed in seeds:
        ...
        control_clean = outputs[CONTROL][seed]["clean"]
        ...
        control, _ = metric_rows(control_clean, "scores", labels, indices)
        arrays[seed] = {
            "primary": primary,
            "unprojected": control,
            "d2p": base,
    ...
    averaged = {
        ...
        for name in ("primary", "unprojected", "d2p", "wrong")
    }
    ...
    comparisons = compare(
        ...
        {
            "d2p": averaged["d2p"],
            "unprojected": averaged["unprojected"],
        },
    ...
    seed_over_control = {
        str(seed): float(
            (arrays[seed]["primary"] - arrays[seed]["unprojected"]).mean()
        )
        for seed in seeds
    }
    ...
    gate = config["gate"]
    d2p = comparisons["d2p"]
    control = comparisons["unprojected"]
    checks = {
        ...
        "over_unprojected_effect": control["mean"]
        >= float(gate["tangent_minus_unprojected_ndcg10_min"]),
        "over_unprojected_ci": control["percentile_95_ci"][0] > 0,
        "over_unprojected_all_seeds": all(
            value > 0 for value in seed_over_control.values()
        ),
        "over_unprojected_all_folds": all(
            row["mean_difference"] > 0 for row in control["hash_folds"]
        ),
    ...
    return {
        ...
        "primary_minus_unprojected": control,
        "seed_primary_minus_d2p": seed_over_d2p,
        "seed_primary_minus_unprojected": seed_over_control,
...
def aggregate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C33 train-gate report exists")
    attempt_path = root / "formal_attempt.json"
    attempt = read_json(attempt_path)
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C33 aggregate attempt state differs")
    store = FrozenTransportStore(config)
    hashes = candidate_hashes(store)
    if hashes != attempt["candidate_hashes"]:
        raise RuntimeError("C33 candidate hashes changed before aggregate")
    seeds = [int(value) for value in config["training"]["seeds"]]
    ...
    for mode in MODES:
        for seed in seeds:
            ...
            if (
                ...
                raise ValueError("C33 seed report identity/boundary differs")
            if row.get("candidate_hashes") != hashes:
                raise RuntimeError("C33 seed candidate hash differs")
    ...
    control_average = average_rows(
        [outputs[CONTROL][seed]["clean"]["scores"] for seed in seeds]
    )
    ...
    tangent_control_activity = order_changes(
        {**common, "scores": control_average},
    ...
    a0_checks = {
        ...
        "paired_initialization": all(
            seed_reports[PRIMARY][seed]["initial_state_sha256"]
            == seed_reports[CONTROL][seed]["initial_state_sha256"]
            for seed in seeds
        ),
        ...
        "capacity_matched_modes": all(
            seed_reports[PRIMARY][seed]["model_identity"]["parameters"]
            == seed_reports[CONTROL][seed]["model_identity"]["parameters"]
            for seed in seeds
        ),
        "primary_uses_tangent": all(
            seed_reports[PRIMARY][seed]["model_identity"]["uses_tangent_projection"]
            is True
            for seed in seeds
        ),
        "control_is_unprojected": all(
            seed_reports[CONTROL][seed]["model_identity"]["uses_tangent_projection"]
            is False
            for seed in seeds
        ),
        ...
        "tangent_changes_unprojected_order": tangent_control_activity["any_fraction"]
        >= float(gate["tangent_vs_unprojected_order_change_fraction_min"]),
        "tangent_changes_unprojected_top10": tangent_control_activity["top10_fraction"]
        >= float(gate["tangent_vs_unprojected_top10_change_fraction_min"]),
        ...
        "selection_integrity": (
            selection_checks["c33_internal_A_features_scores_labels_opened"] is False
            and selection_checks["c33_delayed_B_features_scores_labels_opened"] is False
            and selection_checks["c32_A_labels_previously_opened_but_not_reused"] is True
            and selection_checks["c32_delayed_B_and_escrow_not_reused"] is True
            ...
            and selection_checks["c33_code_dev_test_qrels_metrics_read"] is False
        ),
    }
    a0 = {
        ...
        "tangent_vs_unprojected_order_changes": tangent_control_activity,
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C33 candidate hashes changed before A1 label access")
    ...
    status = (
        "passed_A1_tangent_confirmed_delayed_B_authorized"
        if a1["status"] == "passed"
