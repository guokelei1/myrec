"""Verify the immutable C33 proposal and post-G0 execution locks."""
...
def verify_proposal_lock(config: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if not path.is_file():
        raise PermissionError("C33 proposal lock missing")
    lock = read_json(path)
    if lock.get("lock_id") != "c33_tangent_transport_confirmation_v1":
        raise ValueError("unexpected C33 proposal lock")
    if lock.get("status") != "locked_before_c33_A_feature_label_score_or_outcome":
        raise ValueError("C33 proposal lock stage differs")
    ...
    if failures:
        raise RuntimeError(f"C33 proposal lock mismatch: {failures}")

    declarations = lock["declarations"]
    if declarations.get("c33_internal_A_features_labels_scores_opened") is not False:
        raise ValueError("C33 proposal lock is not pre-A")
    if declarations.get("c32_A_or_reserved_roles_reused_for_c33_gate") is not False:
        raise ValueError("C33 proposal lock reuses the diagnostic outcome cohort")
    if declarations.get("c33_code_dev_test_qrels_metrics_read") is not False:
        raise ValueError("C33 method-boundary declaration differs")
...
def verify_execution_lock(
    ...
    if not path.is_file():
        raise PermissionError("C33 execution lock missing")
    lock = read_json(path)
    if lock.get("lock_id") != "c33_tangent_transport_confirmation_execution_v1":
        raise ValueError("unexpected C33 execution lock")
    if lock.get("status") != "locked_after_G0_before_training_or_A_score":
        raise ValueError("C33 execution lock stage differs")
    if lock.get("proposal_lock_sha256") != proposal_hash:
        raise ValueError("C33 execution lock proposal mismatch")
    report_path = Path(config["paths"]["artifact_root"]) / "g0_report.json"
    if sha256_file(report_path) != lock["g0_report_sha256"]:
        raise RuntimeError("C33 G0 report changed")
    ...
    for name, metadata in report["outputs"].items():
        ...
        if expected != metadata["sha256"] or sha256_file(metadata["path"]) != expected:
            raise RuntimeError(f"C33 G0 output changed: {name}")
    return lock, sha256_file(path)
