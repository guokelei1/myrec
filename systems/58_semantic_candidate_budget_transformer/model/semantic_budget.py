"""Fixed-direction semantic candidate-budget Transformer for C58.
...
class SemanticCandidateBudgetTransformer(nn.Module):
    ...
    def __init__(self, *, null_logit: float = 0.0, epsilon: float = 1e-6) -> None:
        super().__init__()
        if epsilon <= 0:
            raise ValueError("C58 epsilon must be positive")
    ...
    def _zscore(self, values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        for row in range(len(values)):
            ...
            if not len(positions):
                continue
            selected = values[row, positions].float()
            scale = selected.std(correction=0)
            if bool(torch.isfinite(scale)) and float(scale) > self.epsilon:
                output[row, positions] = (selected - selected.mean()) / scale
        return output
        ...
        *,
        ...
        for row in range(batch):
            ...
            if bool(candidate_content.any(dim=-1).logical_not().any()):
                raise ValueError("C58 valid candidate lacks content token")
            ...
            if not bool(torch.isfinite(total)) or float(total) <= self.epsilon:
                raise ValueError("C58 active history lacks positive finite event weight")
        ...
        for row in torch.where(active)[0]:
            ...
            probability = torch.softmax(torch.cat((event_logits, null), dim=-1), dim=-1)
            ...
            no_null = torch.softmax(event_logits, dim=-1)
