"""Create immutable C34 proposal and post-G0 execution locks."""
...
def external_paths(config: dict[str, Any]) -> list[Path]:
    ...
    if outside:
        raise ValueError(f"C34 external input outside repository: {outside}")
    return result
...
def freeze_proposal(config: dict[str, Any]) -> dict[str, Any]:
    ...
    value = {
        ...
        "declarations": {
            "c33_fit_labels_previously_opened_but_not_reused": True,
            "c33_A_labels_previously_opened_for_diagnostic": True,
            "c33_or_c32_target_roles_reused_for_c34_gate": False,
            "prior_label_free_wrong_donors_may_be_reused": True,
            "c33_delayed_B_features_labels_scores_opened": False,
            "c34_fit_new_labels_opened": False,
            "c34_internal_A_features_labels_scores_opened": False,
            "c34_delayed_B_features_labels_scores_opened": False,
            "c34_ranking_outcome_observed": False,
        ...
        "lock_id": "c34_candidate_tangent_cone_gate_v1",
        "selection_path": relative_repo(config["paths"]["selection"]),
        "selection_sha256": config["paths"]["selection_sha256"],
        "status": "locked_before_c34_A_feature_label_score_or_outcome",
    }
...
def freeze_execution(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if report.get("status") != "passed":
        raise PermissionError("C34 G0 is not a pass")
    if report.get("internal_A_labels_opened") is not False:
        raise PermissionError("C34 A labels opened before execution lock")
    if report.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C34 delayed-B boundary differs")
    value = {
        "candidate_id": "c34",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "declarations": {
        ...
        "lock_id": "c34_candidate_tangent_cone_execution_v1",
