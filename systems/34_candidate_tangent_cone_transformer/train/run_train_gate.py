"""Parallel-seed, staged C34 train-only architecture gate."""
...
def assert_cuda(config: Mapping[str, Any], seed: int, device: torch.device) -> int:
    ...
    if seed not in seeds:
        raise ValueError(f"unregistered C34 seed: {seed}")
    physical = physicals[seeds.index(seed)]
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C34 seed/GPU registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C34 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C34 seed process requires exactly one visible GPU")
    return physical
...
def model_identity(model: LowRankTangentConeAttention) -> dict[str, Any]:
    return {
        "architecture": {
            PRIMARY: "candidate_specific_tangent_cone_attention",
            TARGET_CONTROL: "standard_candidate_target_attention",
            GLOBAL_CONTROL: "candidate_shared_global_tangent_transport",
        }[model.mode],
        "mode": model.mode,
        "candidate_specific_transport": model.mode in {PRIMARY, TARGET_CONTROL},
        "positive_tangent_cone": model.mode == PRIMARY,
        "forced_softmax_history_assignment": model.mode == TARGET_CONTROL,
...
def training_rows(
    ...
    for index in store.role_indices("fit"):
        ...
        if stop - start != count:
            raise ValueError("C34 fit label count differs")
...
def train_primary(
    model: LowRankTangentConeAttention,
    store: FrozenTransportStore,
    ...
    if listwise_weight <= 0 or direction_weight <= 0:
        raise ValueError("C34 fixed loss weights must be positive")
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("nonfinite C34 training loss")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C34 gradient: {name}")
...
def write_npz_once(path: Path, **arrays: np.ndarray) -> None:
    if path.exists():
        raise FileExistsError(f"immutable C34 score artifact exists: {path}")
...
def candidate_hashes(store: FrozenTransportStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C34 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C34 checkpoint exists: {path}")
    temporary = path.with_suffix(path.suffix + ".tmp")
    torch.save(
        {
            "candidate_id": "c34",
...
def cone_diagnostics(
    ...
    if not active_requests or not candidate_total:
        raise RuntimeError("C34 cone diagnostic surface empty")
    return {
...
def prepare(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if attempt_path.exists() or report_path.exists():
        raise FileExistsError("C34 formal attempt already exists")
...
def run_seed(
    ...
    if mode not in MODES:
        raise ValueError(f"unregistered C34 mode: {mode}")
    ...
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C34 formal attempt not prepared")
    ...
    if report_path.exists() or score_path.exists():
        raise FileExistsError(f"C34 seed output exists: {seed}")
    started = time.time()
    store = FrozenTransportStore(config)
    if candidate_hashes(store) != attempt["candidate_hashes"]:
        raise RuntimeError("C34 candidate hashes changed before seed training")
    labels = fit_labels(config)
    model = make_model(config, seed, mode)
    identity = model_identity(model)
    if not identity["up_projection_initialized_exact_zero"]:
        raise RuntimeError("C34 neutral adapter initialization differs")
    ...
    cone_audit = cone_diagnostics(model, store, A_indices, config, device)
...
def load_seed_scores(report: Mapping[str, Any], store: FrozenTransportStore) -> dict[str, Any]:
    ...
    if sha256_file(path) != report["score_artifact"]["sha256"]:
        raise RuntimeError("C34 seed score artifact changed")
    ...
    if indices != expected:
        raise RuntimeError("C34 seed score request order differs")
...
def utility_gate(
    *,
    ...
    for seed in seeds:
        ...
        for mode in (TARGET_CONTROL, GLOBAL_CONTROL):
    ...
    averaged = {
        ...
        for name in ("primary", "d2p", "wrong", TARGET_CONTROL, GLOBAL_CONTROL)
    }
    ...
    comparisons = compare(
        ...
        {
            "d2p": averaged["d2p"],
            TARGET_CONTROL: averaged[TARGET_CONTROL],
            GLOBAL_CONTROL: averaged[GLOBAL_CONTROL],
        },
    ...
    for mode in (TARGET_CONTROL, GLOBAL_CONTROL):
        ...
        checks[f"over_{mode}_effect"] = comparison["mean"] >= float(
            gate["cone_minus_each_control_ndcg10_min"]
        )
    ...
    return {
        ...
        "primary_minus_controls": {
            mode: comparisons[mode] for mode in (TARGET_CONTROL, GLOBAL_CONTROL)
        },
...
def aggregate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C34 train-gate report exists")
    attempt_path = root / "formal_attempt.json"
    attempt = read_json(attempt_path)
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C34 aggregate attempt state differs")
    store = FrozenTransportStore(config)
    hashes = candidate_hashes(store)
    if hashes != attempt["candidate_hashes"]:
        raise RuntimeError("C34 candidate hashes changed before aggregate")
    seeds = [int(value) for value in config["training"]["seeds"]]
    ...
    for mode in MODES:
        for seed in seeds:
            ...
            if (
                ...
                raise ValueError("C34 seed report identity/boundary differs")
            if row.get("candidate_hashes") != hashes:
                raise RuntimeError("C34 seed candidate hash differs")
    ...
    a0_checks = {
        ...
        "primary_uses_positive_tangent_cone": all(
            seed_reports[PRIMARY][seed]["model_identity"]["positive_tangent_cone"]
            is True
            for seed in seeds
        ),
        "target_control_forces_softmax_assignment": all(
            seed_reports[TARGET_CONTROL][seed]["model_identity"][
                "forced_softmax_history_assignment"
            ]
            is True
            for seed in seeds
        ),
        ...
        "cone_changes_each_control_order": all(
            row["any_fraction"]
            >= float(gate["cone_vs_each_control_order_change_fraction_min"])
            for row in control_activities.values()
        ),
        "cone_changes_each_control_top10": all(
            row["top10_fraction"]
            >= float(gate["cone_vs_each_control_top10_change_fraction_min"])
            for row in control_activities.values()
        ),
        "cone_has_exact_zero_support_candidates": all(
            float(
                seed_reports[PRIMARY][seed]["cone_diagnostics"][
                    "zero_support_candidate_fraction"
                ]
            )
            >= float(gate["cone_zero_support_candidate_fraction_min"])
            for seed in seeds
        ),
        "cone_has_candidate_distinct_displacements": all(
            float(
                seed_reports[PRIMARY][seed]["cone_diagnostics"][
        ...
        "selection_integrity": (
            selection_checks["c34_internal_A_features_scores_labels_opened"] is False
            and selection_checks["c34_delayed_B_features_scores_labels_opened"] is False
            and selection_checks["fresh_fit_not_reused"] is True
            and selection_checks["c33_A_labels_previously_opened_but_not_reused"] is True
            and selection_checks["c33_delayed_B_and_escrow_not_reused"] is True
            and selection_checks["c32_c33_target_roles_not_reused"] is True
            and selection_checks[
                "prior_wrong_donors_may_become_targets_without_prior_target_exposure"
            ]
            is True
            ...
            and selection_checks["c34_code_dev_test_qrels_metrics_read"] is False
        ),
    }
    a0 = {
        ...
        "cone_vs_control_order_changes": control_activities,
        "seed_cone_diagnostics": {
            str(seed): seed_reports[PRIMARY][seed]["cone_diagnostics"]
            for seed in seeds
        },
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C34 candidate hashes changed before A1 label access")
    ...
    status = (
        "passed_A1_candidate_tangent_cone_delayed_B_authorized"
        if a1["status"] == "passed"
