"""Materialize label-free C34-A features and compact fit labels after proposal lock."""
...
def assert_cuda(config: Mapping[str, Any], device: str) -> None:
    ...
    if device != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C34 G0 GPU registration mismatch")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C34 G0 requires exactly one visible CUDA GPU")
...
def validate_registered_sources(config: Mapping[str, Any]) -> None:
    ...
    for name, expected_name in (
        ...
        if sha256_file(paths[name]) != paths[expected_name]:
            raise ValueError(f"C34 registered source changed: {name}")
    c33_g0 = read_json(paths["c33_g0_report"])
    c33_outcome = read_json(paths["c33_train_report"])
    if c33_g0.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C34 source reserved role was materialized")
    if c33_outcome.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C34 source reserved boundary differs")
    if c33_outcome.get("status") != "failed_A1_terminal":
        raise PermissionError("C34 C33 terminal state differs")
    if c33_outcome.get("internal_A_labels_opened") is not True:
        raise PermissionError("C34 C33 diagnostic cohort state differs")
...
def materialize(config_path: str | Path, device: str) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C34 G0 exists")
    ...
    if len(feature_indices) != len(set(int(value) for value in feature_indices)):
        raise AssertionError("C34 feature roles overlap")
    ...
    if checkpoint.get("analysis_id") != "finetuned_nonpersonalized_control_v1":
        raise ValueError("C34 D2 checkpoint identity differs")
    if int(checkpoint.get("seed", -1)) != 20260708:
        raise ValueError("C34 D2 checkpoint seed differs")
    model = build_model(d2, device)
    model.load_state_dict(checkpoint["model_state"], strict=True)
    model.eval()
    # D2p keeps its registered fine-tuned query tower.  C34 transport must use
    ...
    if sha256_file(paths["internal_train_popularity"]) != config["integrity"][
        ...
        raise ValueError("C34 popularity differs")
        ...
        for start in range(0, len(feature_indices), batch_size):
            ...
            for row, raw_index in enumerate(requests):
                ...
                if not np.array_equal(item_indices[positions], candidates):
                    raise RuntimeError("C34 selected item lookup differs")
                ...
                if not np.isfinite(mixed).all():
                    raise ValueError("nonfinite C34 D2p")
