"""C34 frozen selection, packed structure, and immutable-file utilities."""
...
def write_json_once(path: str | Path, value: Mapping[str, Any]) -> None:
    target = Path(path)
    if target.exists():
        raise FileExistsError(f"immutable C34 output exists: {target}")
...
def load_config(path: str | Path, *, require_selection: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("candidate_id") != "c34":
        raise ValueError("unexpected C34 config")
    if config.get("gate_id") != "c34_candidate_tangent_cone_gate_v1":
        raise ValueError("unexpected C34 gate")
    actual = {
    ...
    if actual != ROLE_COUNTS or physical != [0, 1, 2]:
        raise ValueError("C34 frozen role/GPU registration differs")
    if config["model"]["modes"] != [
        "candidate_tangent_cone",
        "standard_target_attention",
        "global_tangent_transport",
    ]:
        raise ValueError("C34 frozen mode registration differs")
    if [int(value) for value in config["training"]["seeds"]] != [
        20260961,
        20260962,
        20260963,
    ]:
        raise ValueError("C34 frozen seed registration differs")
...
class PackedStructure:
    def __init__(self, root: str | Path) -> None:
        ...
        if len(self.request_ids) + 1 != len(self.candidate_offsets):
            raise ValueError("C34 packed request/candidate mismatch")
        if len(self.request_ids) + 1 != len(self.history_offsets):
            raise ValueError("C34 packed request/history mismatch")
        if len(self.request_ids) != len(self.timestamps):
            raise ValueError("C34 packed request/timestamp mismatch")
...
def stable_key(seed: int, role: str, request_id: str) -> tuple[bytes, str]:
    payload = (
        b"c34-candidate-tangent-cone-transformer-v1\0"
