"""C42 frozen embedding store and role-scoped label opener."""
...
class CompactLabels:
    ...
    def __post_init__(self) -> None:
        if len(self.offsets) != len(self.request_indices) + 1:
            raise ValueError("C42 label offsets differ")
        if int(self.offsets[-1]) != len(self.values):
            raise ValueError("C42 label values differ")
    ...
    def row(self, index: int, expected_count: int) -> np.ndarray:
        ...
        if position is None:
            raise PermissionError(f"C42 labels unavailable for {index}")
        start, stop = int(self.offsets[position]), int(self.offsets[position + 1])
        if stop - start != expected_count:
            raise ValueError("C42 label candidate count differs")
...
class FrozenStore:
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        if not manifest.get("finite", False):
            raise RuntimeError("C42 embeddings are not finite")
        ...
        if len(self.feature_position) != len(self.feature_indices):
            raise ValueError("C42 feature indices overlap")
    ...
    def role_indices(self, role: str) -> list[int]:
        ...
        if any(index not in self.feature_position for index in indices):
            raise PermissionError(f"C42 role {role} has no features")
        return indices
    ...
    def row_position(self, index: int) -> int:
        try:
            ...
            raise PermissionError(f"C42 feature unavailable for {index}") from exc
    ...
    def history_positions(self, index: int, source: str) -> np.ndarray:
        ...
        raise ValueError(f"unknown C42 history source: {source}")
...
def open_role_labels(
    *,
    ...
    if sha256_file(records_train_path) != records_train_sha256:
        raise RuntimeError("C42 label-bearing records changed")
    if sha256_file(selection_path) != selection_sha256:
        raise RuntimeError("C42 selection changed")
    ...
    with path.open("r", encoding="utf-8") as handle:
        for expected, line in enumerate(handle):
            ...
            if int(row["position"]) != expected:
                raise ValueError(f"C42 {path.name} positions differ")
