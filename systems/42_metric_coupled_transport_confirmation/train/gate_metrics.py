"""Frozen C42 paired-bootstrap and ranking helpers."""
...
def bootstrap(values: np.ndarray, *, samples: int, seed: int) -> dict[str, Any]:
    ...
    if values.ndim != 1 or not len(values) or not np.isfinite(values).all():
        raise ValueError("C42 bootstrap values invalid")
    ...
    for offset, (name, reference) in enumerate(references.items()):
        ...
        for fold in range(folds):
            selected = np.asarray(
                [
                    int.from_bytes(
                        hashlib.sha256(f"c42-fold:{seed}:{request_id}".encode()).digest()[:8],
                        "big",
                    )
                    % folds
                    == fold
...
def clicked_direction(
    ...
    if not output:
        raise ValueError("C42 clicked-direction surface empty")
