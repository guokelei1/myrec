"""Parallel-seed, staged C35 train-only architecture gate."""
...
CONTROL_MODES = (ABSOLUTE_CONTROL, AXIS_SOFTMAX_CONTROL, GLOBAL_CONTROL)
...
def assert_cuda(config: Mapping[str, Any], seed: int, device: torch.device) -> int:
    ...
    if seed not in seeds:
        raise ValueError(f"unregistered C35 seed: {seed}")
    physical = physicals[seeds.index(seed)]
    if str(device) != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C35 seed/GPU registration mismatch")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C35 deterministic CUBLAS setting absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C35 seed process requires exactly one visible GPU")
    return physical
...
def model_identity(model: LowRankRelativeSurplusAttention) -> dict[str, Any]:
    return {
        "architecture": {
            PRIMARY: "candidate_relative_tangent_surplus_attention",
            ABSOLUTE_CONTROL: "absolute_candidate_tangent_cone",
            AXIS_SOFTMAX_CONTROL: "candidate_axis_softmax_attention",
            GLOBAL_CONTROL: "candidate_shared_global_tangent_transport",
        }[model.mode],
        "mode": model.mode,
        "candidate_specific_transport": model.mode != GLOBAL_CONTROL,
        "candidate_relative_surplus": model.mode == PRIMARY,
        "absolute_tangent_cone": model.mode == ABSOLUTE_CONTROL,
        "candidate_axis_softmax": model.mode == AXIS_SOFTMAX_CONTROL,
...
def training_rows(
    ...
    for index in store.role_indices("fit"):
        ...
        if stop - start != count:
            raise ValueError("C35 fit label count differs")
...
def train_primary(
    model: LowRankRelativeSurplusAttention,
    store: FrozenTransportStore,
    ...
    if listwise_weight <= 0 or direction_weight <= 0:
        raise ValueError("C35 fixed loss weights must be positive")
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("nonfinite C35 training loss")
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None:
                    if not bool(torch.isfinite(parameter.grad).all()):
                        raise RuntimeError(f"nonfinite C35 gradient: {name}")
...
def write_npz_once(path: Path, **arrays: np.ndarray) -> None:
    if path.exists():
        raise FileExistsError(f"immutable C35 score artifact exists: {path}")
...
def candidate_hashes(store: FrozenTransportStore) -> dict[str, str]:
    ...
    for role, row in store.selection["roles"].items():
        ...
        if actual != row["candidate_key_sha256"]:
            raise RuntimeError(f"C35 candidate hash differs: {role}")
        output[role] = actual
    return output
...
def save_checkpoint(
    ...
    if path.exists():
        raise FileExistsError(f"C35 checkpoint exists: {path}")
    temporary = path.with_suffix(path.suffix + ".tmp")
    torch.save(
        {
            "candidate_id": "c35",
...
def surplus_diagnostics(
    ...
    with torch.inference_mode():
        for index in indices:
            ...
            _, _, displacement, _, support = model.transport_state(
    ...
    if not active_requests or not candidate_total:
        raise RuntimeError("C35 surplus diagnostic surface empty")
    return {
...
def prepare(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if attempt_path.exists() or report_path.exists():
        raise FileExistsError("C35 formal attempt already exists")
...
def run_seed(
    ...
    if mode not in MODES:
        raise ValueError(f"unregistered C35 mode: {mode}")
    ...
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C35 formal attempt not prepared")
    ...
    if report_path.exists() or score_path.exists():
        raise FileExistsError(f"C35 seed output exists: {seed}")
    started = time.time()
    store = FrozenTransportStore(config)
    if candidate_hashes(store) != attempt["candidate_hashes"]:
        raise RuntimeError("C35 candidate hashes changed before seed training")
    labels = fit_labels(config)
    model = make_model(config, seed, mode)
    identity = model_identity(model)
    if not identity["up_projection_initialized_exact_zero"]:
        raise RuntimeError("C35 neutral adapter initialization differs")
    ...
    surplus_audit = surplus_diagnostics(model, store, A_indices, config, device)
    ...
    output = {
        ...
        "surplus_diagnostics": surplus_audit,
...
def load_seed_scores(report: Mapping[str, Any], store: FrozenTransportStore) -> dict[str, Any]:
    ...
    if sha256_file(path) != report["score_artifact"]["sha256"]:
        raise RuntimeError("C35 seed score artifact changed")
    ...
    if indices != expected:
        raise RuntimeError("C35 seed score request order differs")
    ...
    *,
    ...
    for mode in CONTROL_MODES:
        ...
        checks[f"over_{mode}_effect"] = comparison["mean"] >= float(
            gate["surplus_minus_each_control_ndcg10_min"]
        )
...
def aggregate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if report_path.exists():
        raise FileExistsError("immutable C35 train-gate report exists")
    attempt_path = root / "formal_attempt.json"
    attempt = read_json(attempt_path)
    if attempt.get("status") != "prepared_before_training_or_internal_A_score":
        raise PermissionError("C35 aggregate attempt state differs")
    store = FrozenTransportStore(config)
    hashes = candidate_hashes(store)
    if hashes != attempt["candidate_hashes"]:
        raise RuntimeError("C35 candidate hashes changed before aggregate")
    seeds = [int(value) for value in config["training"]["seeds"]]
    ...
    for mode in MODES:
        for seed in seeds:
            ...
            if (
                ...
                raise ValueError("C35 seed report identity/boundary differs")
            if row.get("candidate_hashes") != hashes:
                raise RuntimeError("C35 seed candidate hash differs")
    ...
    a0_checks = {
        ...
        "primary_uses_candidate_relative_surplus": all(
            seed_reports[PRIMARY][seed]["model_identity"]["candidate_relative_surplus"]
            is True
            for seed in seeds
        ),
        "absolute_control_is_predecessor_cone": all(
            seed_reports[ABSOLUTE_CONTROL][seed]["model_identity"][
                "absolute_tangent_cone"
            ]
            is True
            for seed in seeds
        ),
        "axis_softmax_control_is_registered": all(
            seed_reports[AXIS_SOFTMAX_CONTROL][seed]["model_identity"][
        ...
        "surplus_changes_each_control_order": all(
            row["any_fraction"]
            >= float(gate["surplus_vs_each_control_order_change_fraction_min"])
            for row in control_activities.values()
        ),
        "surplus_changes_each_control_top10": all(
            row["top10_fraction"]
            >= float(gate["surplus_vs_each_control_top10_change_fraction_min"])
            for row in control_activities.values()
        ),
        "surplus_has_exact_zero_support_candidates": all(
            float(
                seed_reports[PRIMARY][seed]["surplus_diagnostics"][
        ...
        "surplus_has_mixed_admission_requests": all(
            float(
                seed_reports[PRIMARY][seed]["surplus_diagnostics"][
        ...
        "surplus_has_candidate_distinct_displacements": all(
            float(
                seed_reports[PRIMARY][seed]["surplus_diagnostics"][
        ...
        "selection_integrity": (
            selection_checks["c35_internal_A_features_scores_labels_opened"] is False
            and selection_checks["c35_delayed_B_features_scores_labels_opened"] is False
            and selection_checks["c34_fit_reused_with_labels_previously_opened"] is True
            and selection_checks["c34_A_features_scores_opened_but_not_reused"] is True
            and selection_checks["c34_A_labels_opened"] is False
            and selection_checks["c34_delayed_B_promoted_to_c35_A_unopened"] is True
            and selection_checks["c34_escrow_promoted_to_c35_B_unopened"] is True
            and selection_checks["fresh_c35_escrow_and_structural_roles"] is True
            ...
            and selection_checks["c35_code_dev_test_qrels_metrics_read"] is False
        ),
    }
    a0 = {
        ...
        "surplus_vs_control_order_changes": control_activities,
        "seed_surplus_diagnostics": {
            str(seed): seed_reports[PRIMARY][seed]["surplus_diagnostics"]
            for seed in seeds
        },
    ...
    if candidate_hashes(store) != hashes:
        raise RuntimeError("C35 candidate hashes changed before A1 label access")
    ...
    status = (
        "passed_A1_candidate_relative_surplus_delayed_B_authorized"
        if a1["status"] == "passed"
