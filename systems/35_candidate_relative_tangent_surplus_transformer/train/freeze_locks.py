"""Create immutable C35 proposal and post-G0 execution locks."""
...
def external_paths(config: dict[str, Any]) -> list[Path]:
    ...
    if outside:
        raise ValueError(f"C35 external input outside repository: {outside}")
    return result
...
def freeze_proposal(config: dict[str, Any]) -> dict[str, Any]:
    ...
    value = {
        ...
        "declarations": {
            "c34_fit_labels_previously_opened_and_reused": True,
            ...
            "c35_fit_labels_opened_in_this_attempt": False,
            "c35_internal_A_features_labels_scores_opened": False,
            "c35_delayed_B_features_labels_scores_opened": False,
            "c35_ranking_outcome_observed": False,
        ...
        "lock_id": "c35_candidate_relative_surplus_gate_v1",
        "selection_path": relative_repo(config["paths"]["selection"]),
        "selection_sha256": config["paths"]["selection_sha256"],
        "status": "locked_before_c35_A_feature_label_score_or_outcome",
    }
...
def freeze_execution(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if report.get("status") != "passed":
        raise PermissionError("C35 G0 is not a pass")
    if report.get("internal_A_labels_opened") is not False:
        raise PermissionError("C35 A labels opened before execution lock")
    if report.get("delayed_B_features_labels_scores_opened") is not False:
        raise PermissionError("C35 delayed-B boundary differs")
    value = {
        "candidate_id": "c35",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "declarations": {
        ...
        "lock_id": "c35_candidate_relative_surplus_execution_v1",
