"""C35 frozen selection, packed structure, and immutable-file utilities."""
...
def write_json_once(path: str | Path, value: Mapping[str, Any]) -> None:
    target = Path(path)
    if target.exists():
        raise FileExistsError(f"immutable C35 output exists: {target}")
...
def load_config(path: str | Path, *, require_selection: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("candidate_id") != "c35":
        raise ValueError("unexpected C35 config")
    if config.get("gate_id") != "c35_candidate_relative_surplus_gate_v1":
        raise ValueError("unexpected C35 gate")
    actual = {
    ...
    if actual != ROLE_COUNTS or physical != [0, 1, 2]:
        raise ValueError("C35 frozen role/GPU registration differs")
    if config["model"]["modes"] != [
        ...
        raise ValueError("C35 frozen mode registration differs")
    if [int(value) for value in config["training"]["seeds"]] != [
        20260981,
        20260982,
        20260983,
    ]:
        raise ValueError("C35 frozen seed registration differs")
...
class PackedStructure:
    def __init__(self, root: str | Path) -> None:
        ...
        if len(self.request_ids) + 1 != len(self.candidate_offsets):
            raise ValueError("C35 packed request/candidate mismatch")
        if len(self.request_ids) + 1 != len(self.history_offsets):
            raise ValueError("C35 packed request/history mismatch")
        if len(self.request_ids) != len(self.timestamps):
            raise ValueError("C35 packed request/timestamp mismatch")
...
def stable_key(seed: int, role: str, request_id: str) -> tuple[bytes, str]:
    payload = (
        b"c35-candidate-relative-tangent-surplus-v1\0"
