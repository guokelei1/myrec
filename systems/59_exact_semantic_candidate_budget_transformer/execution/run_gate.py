"""Run C59's locked label-free mechanics gate and conditional utility gate."""
...
def assert_sources(config: Mapping[str, Any]) -> None:
    for path_name, hash_name in (
        ...
        ("c58_model_source", "c58_model_source_sha256"),
        ("c58_report", "c58_report_sha256"),
    ):
        if sha256_file(REPO_ROOT / config["paths"][path_name]) != config["integrity"][hash_name]:
            raise RuntimeError(f"C59 registered source changed: {path_name}")
    source = read_json(REPO_ROOT / config["paths"]["c58_report"])
    if source.get("status") != "failed_label_free_mechanics_terminal":
        raise RuntimeError("C59 C58 terminal boundary differs")
    if source.get("fit_train_labels_read") is not False:
        raise PermissionError("C59 inherited train labels are not closed")
    if source.get("fit_holdout_labels_read") is not False:
        raise PermissionError("C59 inherited holdout labels are not closed")
    if source.get("C26_A_B_escrow_dev_test_qrels_opened") is not False:
        raise PermissionError("C59 inherited protected roles are not closed")
...
def assert_cuda(config: Mapping[str, Any], shard_id: int, device_name: str) -> None:
    ...
    if device_name != "cuda:0" or os.environ.get("CUDA_VISIBLE_DEVICES") != str(physical):
        raise RuntimeError("C59 GPU binding differs")
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") not in {":4096:8", ":16:8"}:
        raise RuntimeError("C59 deterministic CUBLAS workspace absent")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("C59 requires exactly one visible GPU")
...
def make_model(config: Mapping[str, Any]) -> SemanticCandidateBudgetTransformer:
    ...
    if model.parameter_count() != 0:
        raise RuntimeError("C59 fixed operator unexpectedly has parameters")
    return model


def max_difference(first: Sequence[np.ndarray], second: Sequence[np.ndarray]) -> float:
    if len(first) != len(second) or not first:
        raise ValueError("C59 comparison rows differ")
...
def run_shard(config_path: str | Path, shard_id: int, device_name: str) -> dict[str, Any]:
    ...
    if shard_id < 0 or shard_id >= shards:
        raise ValueError("C59 shard differs")
    ...
    if actual_hash != expected_hash:
        raise RuntimeError("C59 holdout candidate hash differs")
...
def load_shard(report: Mapping[str, Any]) -> tuple[np.ndarray, dict[str, list[np.ndarray]]]:
    ...
    if sha256_file(path) != report["score_artifact"]["sha256"]:
        raise RuntimeError("C59 score artifact changed")
    ...
    for report in reports:
        ...
        for local, raw_position in enumerate(positions):
            ...
            if position in seen or position < 0 or position >= count:
                raise RuntimeError("C59 shard position coverage differs")
            seen.add(position)
            for name in SCORE_NAMES:
                output[name][position] = rows[name][local]
    if seen != set(range(count)):
        raise RuntimeError("C59 shard position coverage incomplete")
...
def aggregate_a0(config_path: str | Path) -> dict[str, Any]:
    ...
    if actual_hash != store.selection["candidate_key_sha256"]["holdout"]:
        raise RuntimeError("C59 A0 candidate hash differs")
    root = REPO_ROOT / config["paths"]["artifact_root"]
    reports = [
        ...
        terminal.update(
            {
                ...
                "decision": "close_exact_semantic_candidate_budget_on_mechanics",
...
def aggregate_a1(config_path: str | Path) -> dict[str, Any]:
    ...
    if a0.get("status") != "passed_label_free_mechanics":
        raise PermissionError("C59 A1 requires passed A0")
    store = C56Store(config, REPO_ROOT)
    holdout = store.role("holdout")
    actual_hash = store.candidate_hash(holdout)
    if actual_hash != a0["holdout_candidate_key_sha256"]:
        raise RuntimeError("C59 A1 candidate hash differs")
    reports = [
        read_json(root / f"shard_{shard}_report.json")
        for shard in range(int(config["resources"]["num_shards"]))
    ]
    ...
    value = {
        ...
        "gate_id": config["gate_id"],
        "created_at": datetime.now(timezone.utc).isoformat(),
        "status": "passed_exact_semantic_candidate_budget_foundation"
        if passed
        else "failed_exact_semantic_candidate_budget_terminal",
        "decision": "authorize_fresh_dual_domain_trainable_successor"
        if passed
        else "close_exact_semantic_candidate_budget_family",
        "execution_lock_sha256": execution_hash,
        "holdout_candidate_key_sha256": actual_hash,
        "holdout_requests": len(holdout),
...
def main() -> None:
    ...
    if args.stage == "shard":
        if args.shard_id is None:
            raise ValueError("C59 shard stage requires --shard-id")
